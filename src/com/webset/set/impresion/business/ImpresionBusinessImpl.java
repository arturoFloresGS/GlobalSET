...
/**
 * clase business modulo impresion
 * @author Jessica Arelly Cruz Cruz
 * @since 18/05/2011
 * @Modificado Eric Medina Serrato
 * @Fecha 15/12/2015
 */
public class ImpresionBusinessImpl implements ImpresionService{
	...
	public Map<String, Object>consultarChequesPendientes(ConsultaChequesDto dtoCheques){
		...
		try{
			...
			listaCheques = impresionDao.consultarChequesPendientes2(dtoCheques);
			...
			for(int i = 0; i < listaCheques.size(); i ++)
			{
				...
				if(dtoCheques.getPsbImpreContinua().equals("N"))
				{
					...
					listaFirmas = impresionDao.consultarFirmasChequera(cuenta, banco);
			...
			+ "P:Impresion, C:ImpresionBusinessImpl, M:consultarChequesPendientes");
			e.printStackTrace();
		}
		return retorno;
	}
	...
	public List<MovimientoDto>consultarChequesPendientes1(ConsultaChequesDto dtoCheques)
	{
		...
		try{
			...
			bitacora.insertarRegistro(new Date().toString() + " " + Bitacora.getStackTrace(e) + "P:Impresion, C:ImpresionBusinessImpl, M:consultarChequesPendientes1");
			e.printStackTrace();
		}
		return listaCheques;
	}
		...
		String idConfiguracion)
	{
        ...
        if(impresionDao.seleccionarBHabilita().equals("S") &&
            ...
            /* EMS: 13/01/2016 - Se comenta condición de equivalencia ya que no se toma en cuenta para GSalinas.*/
                /*List<CatEquivaleCuentaContableDto> lEquivalencia =
                        impresionDao.consultarEquivalenciaCtaContable(
           ...
           */
			...
			if (!bOptImpresionContinua){
				existenHojas= impresionDao.existeControlPapel(iBanco, "P", "", "", "A");
			}
			else{
				existenHojas = impresionDao.existeControlPapel(iBanco, "C",listCheques.get(0).getIdChequera()+"" , listCheques.get(0).getNoEmpresa()+"", "A");
			}

			if(!existenHojas && bOptImpresionContinua){
				retorno.put("error", "Error: No existen hojas foliadas para la impresion. ");
				return retorno;
			}
			...
			{
				if (!bOptImpresionContinua) {
					totalHojas= impresionDao.totalHojasControlPapel(iBanco, "", "", false); //26-01-2016 Se obtiene el número total de hojas en control papel
					if(totalHojas==0){ //26-01-2016 si no encuentra hojas termina el proceso.
						impresionDao.actualizarEstatusControlPapel(impresionDao.datosControlPapel(iBanco, "P", "", "", "A"), "T"); //Actualiza el estatus de la chequera en A.
						retorno.put("error", "Error: No existen hojas foliadas para la impresion.");
						return retorno;
					...
					/*List<LlenaComboImpresora> listCharolas =
                                        impresionDao.consultarCharolasImpresora(iNoImpresora, iCaja);
                                */
					//Ingreso 1 ya que noImpresora ya no se maneja, ni cajas
					/*List<LlenaComboImpresora> listCharolas = impresionDao.consultarCharolasImpresora(1, iCaja);
					...
					for (int n = 0; n < listCheques.size(); n ++) {
						listCheques.get(n).setFecValor(dtoBusCheques.getFechaCheque());
					...
					if (iL_NumSol > iL_Papel) {
						int totalHoja = impresionDao.totalHojasControlPapel(iBanco, "", "", false);
						...
						//sigChequera = impresionDao.consultarSigChequera(iBanco,"P");
					...
					List<ControlPapelDto> foliosPapel = impresionDao.consultarFoliosPapel(iBanco, iCaja);
					...
					if (checaFolios) {
					        ...
					        bG_Sali = (impresionDao.actualizarCharolas(iBancoCharola, iNoImpresora) < 0);
					}
					...
					int totalhojas=impresionDao.totalHojasControlPapel(iBanco, sChequera, iEmpresa+"", true);
					...
					if(totalhojas==0 || listCheques.size() > totalhojas){
						if(totalhojas==0) //Si no se encontraron hojas se actualiza el estado de control papel.
							impresionDao.actualizarEstatusControlPapel(impresionDao.datosControlPapel(iBanco, "C", sChequera,iEmpresa+ "", "A"), "T");
						retorno.put("error", "Advertencia: El número de cheques en inventario no es suficiente para la impresión");
						return retorno;
					}
					/*List<CatCtaBancoDto> listContinuo = impresionDao.consultarImpresionContinua(iBanco, sChequera);
					...
					List<ConfiguracionChequeDto> listConfigurados = impresionDao.consultarConfiguracionCheques(iBanco, sMoneda, iEmpresa, idConfiguracion);
			...
			if(bOptImpresionContinua){
				cp = impresionDao.datosControlPapel(iBanco, "C", listCheques.get(0).getIdChequera()+"", listCheques.get(0).getNoEmpresa()+"", "A");
			}
			else{
				cp = impresionDao.datosControlPapel(iBanco, "P", "", "", "A");
			}

			int hojas = cp.getFolioInvFin()-cp.getFolioUltImpreso();

			if(hojas<cp.getStock() && hojas >0)
				retorno.put("msgUsuario", "Quedan "+ hojas + " hojas" );

			if(hojas==0){
				impresionDao.actualizarEstatusControlPapel(cp, "T");
				impresionDao.actualizarEstatusControlPapel(cp, "A");

				boolean existe = false;

				if(bOptImpresionContinua)
					existe= impresionDao.existeControlPapel(cp.getIdBanco(), "C", listCheques.get(0).getIdChequera()+"", listCheques.get(0).getNoEmpresa()+"", "A");
				else
					existe=impresionDao.existeControlPapel(iBanco, "P", "", "", "A");
			...
			bitacora.insertarRegistro(new Date().toString() + " " + Bitacora.getStackTrace(e)
			+ "P:Impresion, C:ImpresionBusinessImpl, M:ejecutarImpresionCheques");
			e.printStackTrace();
		}
		return retorno;
	}
	...
	public Map<String, Object> imprimeCheques(List<MovimientoDto> listCheques, 	ConsultaChequesDto dtoBusCheques, String idConfiguracion){
		...
		try {
			...
			for(int li = 0; li < iRenglon; li++) {
			        ...
			        firmas = impresionDao.consultarContarFirmas(iBancoArreglo, sCuentaArreglo);
			...
			*/
			...
			/*                      if (bOptImpresionLaser) {
			                         for(int li = 0; li < iRenglon; li++) {
			                                 ...
			                                 int firma2=0;
			                                String sCuentaArreglo = arrCheques[li][0];
			                        firma1=impresionDao.obtenerFirmantes(iBancoArreglo, sCuentaArreglo, listCheques.get(li).getNoSolicitud()+"", 1);
			                        firma2=impresionDao.obtenerFirmantes(iBancoArreglo, sCuentaArreglo, listCheques.get(li).getNoSolicitud()+"", 2);
			...
			*/
			...
			bitacora.insertarRegistro(new Date().toString() + " " + Bitacora.getStackTrace(e)
			+ "P:Impresion, C:ImpresionBusinessImpl, M:imprimeCheques");
			e.printStackTrace();
		}
		return result;
	}
	...
	private void mandarNumerosDeChequeASap(DT_ImpresionCheques_OBCheques[] convertirADtoSap) {
		try {
			...
			stub.setUsername(
					impresionDao.consultarConfiguraSet(
	...
	private DT_ImpresionCheques_OBCheques[] convertirADtoSap(List<MovimientoDto> listCheques) {
		...
		try {
			...
			for (MovimientoDto mov : listCheques) {
				arreglo[i] = new DT_ImpresionCheques_OBCheques(
						funciones.ajustarLongitudCampo(
								...
								String.valueOf(mov.getNoCheque()), "", "002");
						//String.valueOf(mov.getNoCheque()), "", mov.getInvoiceType());
				i++;
			}
			...
			bitacora.insertarRegistro(new Date().toString() + " " + Bitacora.getStackTrace(e)
			+ "P:Impresion, C:ImpresionBusinessImpl, M:convertirADtoSap");
			e.printStackTrace();
		}return arreglo;
	}
	...
	public int fCambiaImpresora(String sDriver, String sImpresoraAnt, boolean bCambiaPred){
		...
		try
		{
			...
			bitacora.insertarRegistro(new Date().toString() + " " + Bitacora.getStackTrace(e)
			+ "P:Impresion, C:ImpresionBusinessImpl, M:fCambiaImpresora");
			e.printStackTrace();
		}
	...
	public Map<String, Object> progresoImpresion(List<MovimientoDto> listCheques, ConsultaChequesDto dtoBusCheques, String idConfiguracion){
		...
		try
		{
		    ...
		    if (bOptImpresionLaser) {
		    	int iNoImpresora = dtoBusCheques.getNumImpresora();
		    	int iCaja = dtoBusCheques.getIdCaja();
		    	List<LlenaComboImpresora> listCharolas = impresionDao.consultarCharolasImpresora(iNoImpresora, iCaja);
	    		...
	    		if (listCharolas.size() > 0) {
			        ...
			        listConfigurados = impresionDao.consultarConfiguracionCheques(
			        			listCheques.get(0).getIdBanco(), 
			        			listCheques.get(0).getMoneda(),
			        			listCheques.get(0).getNoEmpresa(),
			        			idConfiguracion);
			        
			        String nombreConf = impresionDao.obtieneDescripcionCatCheque(idConfiguracion);
		    ...
		    StringBuilder sbCheques = new StringBuilder();
		    ...
		    for(int i = 0; i < listCheques.size(); i++) {
            	...
            	String fechaPropuestaTmp = "";
            	
            	if(listCheques.get(i).getFecValor()!=null)
            		fechaPropuestaTmp = funciones.ponerFechaSola(listCheques.get(i).getFecValor());
            	...
            	int noPapel = consultarFoliosPapel(iBanco, 1).get(0).getFolioUltImpreso();
            	iAfectados = impresionDao.actualizarMovimiento(fechaHoy, usuario, fechaChequeCapturada,
            													fechaCheque, auxNumChe, noFolioMov, noPapel+1);
				...
				iAfectados = impresionDao.actualizarCaja(caja,noFolioMov);
				...
				if (bOptImpresionLaser){
					//solo para impresion laser
					//26-01-2016
					ControlPapelDto cp = impresionDao.datosControlPapel(iBanco, "P", "", "", "A");
					...
					if (dtoBusCheques.isChkNuevoFormato())
					{
						if(gsTipoImpresora.equals("HP")) {
							resultCheque = rutinaChequeLaserHP(
									i,listCheques, true,
						...
						if(!resultCheque.equals("")){
							...
							sbCheques.append(resultCheque);
						}
					}
					
					//Se actualiza el control de papel
					iAfectados = impresionDao.actualizaControlPapel(cp.getIdControlCheque());
					...
					///insertar en bitacora cheques
					InsertaBitacoraChequesDto insertaBitacoraChequesdto = new InsertaBitacoraChequesDto();
					insertaBitacoraChequesdto.setNoEmpresa(listCheques.get(i).getNoEmpresa());
					insertaBitacoraChequesdto.setFolioDet(listCheques.get(i).getNoFolioDet());
					insertaBitacoraChequesdto.setNoBanco(listCheques.get(i).getIdBanco());
					insertaBitacoraChequesdto.setChequera(listCheques.get(i).getIdChequera());
					insertaBitacoraChequesdto.setNoCheque(listCheques.get(i).getNoCheque());
					insertaBitacoraChequesdto.setImporte(listCheques.get(i).getImporte());
					insertaBitacoraChequesdto.setEstatus("I");
					insertaBitacoraChequesdto.setDivisa("MN");
					//insertaBitacoraChequesdto.setFechaHoy("GETDATE()");
					insertaBitacoraChequesdto.setUsuario(listCheques.get(i).getUsuarioAlta());
					insertaBitacoraChequesdto.setFolio(listCheques.get(i).getNoFolioDet());
					insertaBitacoraChequesdto.setCausa(0);
					insertaBitacoraChequesdto.setNoChequeAnt(listCheques.get(i).getNoCheque());
					insertaBitacoraChequesdto.setCaja(listCheques.get(i).getIdCaja());
					insertaBitacoraChequesdto.setBeneficiario(listCheques.get(i).getBeneficiario());
					insertaBitacoraChequesdto.setConcepto(listCheques.get(i).getConcepto());
					insertaBitacoraChequesdto.setNoDocto("0");
					insertaBitacoraChequesdto.setFecCheque(listCheques.get(i).getFecChequeStr());
					
					int insertaBitacora = impresionDao.insertarBitacoraCheques(insertaBitacoraChequesdto);
					
					if (insertaBitacora <= 0) {
						result.put("error", "Error al insertar en bitacora cheques.");
						return result;
					}
				...
				else
				{
					...
					//iAfectados = impresionDao.actualizaControlPapel(iBanco, caja);

					/*******Actualizar datos en control papel*****/
					ControlPapelDto cp = impresionDao.datosControlPapel(iBanco, "C", listCheques.get(i).getIdChequera()+"", listCheques.get(i).getNoEmpresa()+"", "A");
					int hojasRestantes= cp.getFolioInvFin()-cp.getFolioUltImpreso();
					
					if(hojasRestantes==0){
	            		impresionDao.actualizarEstatusControlPapel(cp, "T");
	            		impresionDao.actualizarEstatusControlPapel(cp, "A");
	            		cp = impresionDao.datosControlPapel(iBanco, "C", listCheques.get(i).getIdChequera()+"", listCheques.get(i).getNoEmpresa()+"", "A");
	            	}
	            	
	            	String importeLetra = "";
	    			importeLetra = funciones.convertirNumeroEnLetra(listCheques.get(i).getImporte());
	            	...
	            	if (dtoBusCheques.getFechaCheque() == null)
	            		dtoBusCheques.setFechaCheque(globalSingleton.getFechaHoy());
	    			...
	    			iAfectados = impresionDao.actualizaCheque(listCheques.get(i).getNoCheque(), listCheques.get(i).getIdChequera());
	    			...
	    			if(tieneNombreEmpresa){
	    				cheque.setNombreEmpresa(impresionDao.obtenerNombreEmpresa(
													listCheques.get(i).getNoEmpresa()));
	        		}else{
		    ...
		    int segs = cal.get(Calendar.SECOND);
		    
			//Se crea el archivo fisico del cheque.
			FileOutputStream fileOutput = new FileOutputStream (ConstantesSet.RUTA_CHEQUES + cal.get(Calendar.DAY_OF_MONTH) + cal.get(Calendar.MONTH) + cal.get(Calendar.YEAR) + "_" + hora + minu + segs + ".txt");
			BufferedOutputStream bufferedOutput = new BufferedOutputStream(fileOutput);
			bufferedOutput.write(sbCheques.toString().getBytes(), 0, sbCheques.length());
			bufferedOutput.close();
			Thread.sleep(1000);
			System.out.println("Cheques creados");
			
			//Se crea el archivo fisico del proceso.
			FileOutputStream fosProceso = new FileOutputStream(ConstantesSet.RUTA_CHEQUES + cal.get(Calendar.DAY_OF_MONTH) + cal.get(Calendar.MONTH) + cal.get(Calendar.YEAR) + "_" + hora + minu + segs + ".bat");
			BufferedOutputStream bosProceso = new BufferedOutputStream(fosProceso);
			
			//String sProcesoLexmark = "more " + ConstantesSet.RUTA_CHEQUES + cal.get(Calendar.DAY_OF_MONTH) + cal.get(Calendar.MONTH) + cal.get(Calendar.YEAR) + "_" + hora + minu + segs + ".txt | lp -d _172_16_4_117 -o raw";
			//String sProcesoLexmark = "more " + ConstantesSet.RUTA_CHEQUES + cal.get(Calendar.DAY_OF_MONTH) + cal.get(Calendar.MONTH) + cal.get(Calendar.YEAR) + "_" + hora + minu + segs + ".txt | lp -d HP_LaserJet_P3010_Series -o raw";
			//String sProcesoLexmark = "type " + ConstantesSet.RUTA_CHEQUES + cal.get(Calendar.DAY_OF_MONTH) + cal.get(Calendar.MONTH) + cal.get(Calendar.YEAR) + "_" + hora + minu + segs + ".txt > lpt1: ";
			
			//String sProcesoLexmark = ConstantesSet.RUTA_CHEQUES + cal.get(Calendar.DAY_OF_MONTH) 
				//+ cal.get(Calendar.MONTH) + cal.get(Calendar.YEAR) + "_" + hora + minu + segs + ".txt";//
			//bosProceso.write(sProcesoLexmark.getBytes(), 0, sProcesoLexmark.length());
			String sProceso = "type " + ConstantesSet.RUTA_CHEQUES + cal.get(Calendar.DAY_OF_MONTH) 
				+ cal.get(Calendar.MONTH) + cal.get(Calendar.YEAR) + "_" + hora + minu + segs + ".txt > lpt1:";
			...
			//Runtime.getRuntime().exec("chmod 777 " + ConstantesSet.RUTA_CHEQUES + cal.get(Calendar.DAY_OF_MONTH) + cal.get(Calendar.MONTH) + cal.get(Calendar.YEAR) + "_" + hora + minu + segs + ".sh");
			String ejecucion =ejecutaComando(ConstantesSet.RUTA_CHEQUES + cal.get(Calendar.DAY_OF_MONTH) + cal.get(Calendar.MONTH) + cal.get(Calendar.YEAR) + "_" + hora + minu + segs + ".bat");
			//String ejecucion = "java -jar " + ConstantesSet.RUTA_CHEQUES + "impresion.jar";
			//Runtime.getRuntime().exec(ejecucion);
			// ejecutaComando(ejecucion);
			escribirArchivo("impresion.log", ejecucion, false);
			Thread.sleep(1000);
			System.out.println("Cheques impresos");
		    logger.info("Cheques impresos");
			...
			bitacora.insertarRegistro(new Date().toString() + " " + Bitacora.getStackTrace(e)
			+ "P:Impresion, C:ImpresionBusinessImpl, M:progresoImpresion");
			e.printStackTrace();
		}
		return result;
	}
	...
	private String ejecutaComando(String comando) {
		...
		try { 
			...
			while (input.readLine() != null) { 
				escribirArchivo("impresion.log", input.readLine(), false); 
			} 
			input.close(); 
			try {
	    		BufferedReader errinput = new BufferedReader(new InputStreamReader(p.getErrorStream()));
	    		result = (errinput.readLine().toString());
	    		errinput.close();
	    		return result; 
			} catch(Exception err) {
				return result;
	...
	private void escribirArchivo(String archivoIn, String texto, boolean borrar) {
		System.out.println("escribe archivo: " + archivoIn + " Borrar: " + borrar);
		if (borrar) {
			if (!borraArchivo(archivoIn)) {
				escribirArchivo("impresion.log", new Date().toString() + " Error al borrar el archivo: " + archivoIn + " \r\n", false);
				return;
			}
		}
		try {
			File archivo = new File(ConstantesSet.RUTA_CHEQUES + archivoIn);
			FileWriter escribir=new FileWriter(archivo,true);
		
			escribir.write(texto);
			escribir.close();
		} catch (Exception e) {
			escribirArchivo("impresion.log", new Date().toString() + " Error al escribir el archivo: " + archivoIn + " \r\n", false);
		}
	}
	
	private boolean borraArchivo(String archivoIn) {
		System.out.println("Borra archivo: " + archivoIn);
		File archivo = new File(ConstantesSet.RUTA_CHEQUES + archivoIn);
		if (archivo.isFile()) {
			if (!ejecutaComando("cmd /c del " + ConstantesSet.RUTA_CHEQUES + archivoIn).equals("")) {
				return false;
			}
		}
		return true;
	}
	/** ----------------RUTINA PARA IMPRESORA HP --------------- **/
	public String rutinaChequeLaserHP(int renglon, List<MovimientoDto> listCheques,
			boolean imprime, boolean negociable, boolean abono, String tipoImpresora)
	{
		...
		try{
			...
			//sBancoTrunc = sBancoTrunc.replaceAll(",", "&#255;");
			sBancoTrunc = sBancoTrunc.replaceAll("Ñ", "N");
			sBancoTrunc = sBancoTrunc.replaceAll("ñ", "n");
			...
			//if(listCheques.get(renglon).getFecCheque() == null)
				listCheques.get(renglon).setFecCheque(listCheques.get(renglon).getFecValor());
			...
			if(Integer.parseInt(sMesActual.substring(6, 10)) > 1999){
				sFechaCheque = sMesActual.substring(0, 2) + " DE " + sMes.toUpperCase() + " DE " + sMesActual.substring(6, 10);
				sFechaCorta = sMesActual.substring(0, 2) + " DE " + sMesCorto.toUpperCase() + " DE " + sMesActual.substring(6, 10);
			}
			else{
				sFechaCheque = sMesActual.substring(0, 2) + " DE " + sMes.toUpperCase() + " DE " + sMesActual.substring(6, 10);
				sFechaCorta = sMesActual.substring(0, 2) + " DE " + sMesCorto.toUpperCase() + " DE " + sMesActual.substring(6, 10);
			...
			sContCheque.append(ConstantesSet.FORMA_CHEQUE_WEB_HP);
			...
			//Numero de Proveedor
			sContCheque.append("@@*p45x350Y" + listCheques.get(renglon).getEquivaleBenef() + " ");
			
			//Nombre del Beneficiario
			String sNomBenef = (listCheques.get(renglon).getBeneficiario().trim()); //.replaceAll(" ", "&#255;");
			sNomBenef = sNomBenef.replaceAll("Ñ", "N");  sNomBenef = sNomBenef.replaceAll("ñ", "n");
			sContCheque.append("@@*p470x350Y" + sNomBenef + " ");
			...
			sContCheque.append(formatoImporte(1925,350,funciones.obtenerFormatoAsteriscos(listCheques.get(renglon).getImporte(), 12, ""), 'N'));
			
			//Pago concepto de
			String sConceptoPago = (listCheques.get(renglon).getConcepto().trim()); //.replaceAll(" ", "&#255;");
			sConceptoPago = sConceptoPago.replaceAll("Ñ", "N");  sConceptoPago = sConceptoPago.replaceAll("ñ", "n");
			sContCheque.append("@@*p45x645Y" + sConceptoPago + " ");
			...
			sContCheque.append(formatoImporte(2020,1170, funciones.obtenerFormatoAsteriscos(listCheques.get(renglon).getImporte(), 12, ""), 'N'));
			
			//Fecha corta
			sContCheque.append("@@*p45x1380Y" + sFechaCorta + " ");
			
			//Banco
			sContCheque.append("@@*p470x1380Y" + sBancoTrunc + " ");
			
			//No Cuenta
			sContCheque.append("@@*p1150x1380Y" + sNoCta + " ");
			
			//Sociedad
			String sSoc = (listCheques.get(renglon).getCiaNmbr()); //.replaceAll(" ", "&#255;");
			sSoc = sSoc.replaceAll("Ñ", "N");   sSoc = sSoc.replaceAll("ñ", "n");  
			//sSoc = sSoc.replaceAll(".", ""); 
			sContCheque.append("@@*p1570x1380Y" + sSoc + " ");
			
			//Tipo de Egreso
			sContCheque.append("@@*p45x1580Y" + listCheques.get(renglon).getIdRubroStr() + " ");
			
			//No Cheque
			sContCheque.append("@@*p690x1580Y" + funciones.ponerCeros(""+listCheques.get(renglon).getNoCheque(), 7) + " ");
			
			//Caja de Pago
			String sTmp = listCheques.get(renglon).getDescCaja();
			if(sTmp!=null && sTmp.length()>17)
				sContCheque.append("@@*p1145x1580Y" + sTmp.substring(0, 18)); //.replaceAll(" ", "&#255;") + " ");
			else
				sContCheque.append("@@*p1145x1580Y" + sTmp);
			
			//Contrarecibo
			sContCheque.append("@@*p1570x1580Y" + listCheques.get(renglon).getNoContrarecibo() + " ");
			
			//Folio Tesoreria SET:
			sContCheque.append("@@*p1990x1580Y" + listCheques.get(renglon).getNoSolicitud() + " ");
			
			//Datos de la empresa
			sContCheque.append("@@(s1p1h10v0s+0b4T ");
			//Nombre empresa
			String sNomComp = (listCheques.get(renglon).getCiaNmbr()); //.replaceAll(" ", "&#255;");
			sNomComp = sNomComp.replaceAll("Ñ", "N");   sNomComp = sNomComp.replaceAll("ñ", "n"); 
			//sNomComp = sNomComp.replaceAll(".", "");
			sContCheque.append("@@*p950x2107Y" + sNomComp);
			
			//Direccion de la empresa
			String sDir1 = (listCheques.get(renglon).getCiaDir()); //.replaceAll(" ", "&#255;");
			sDir1.replaceAll("Ñ", "N");   sDir1.replaceAll("ñ", "n");
			String sDir2 = (listCheques.get(renglon).getCiaDir2()); //.replaceAll(" ", "&#255;");
			sDir2.replaceAll("Ñ", "N");   sDir2.replaceAll("ñ", "n");
			
			sContCheque.append("@@(s1p1h6v0s+0b4T ");
			sContCheque.append("@@*p950x2155Y" + sDir1.substring(0, sDir1.indexOf('|')) + " ");
			sContCheque.append("@@*p950x2190Y" + sDir1.substring(sDir1.indexOf('|')+1, sDir1.length()) + " ");
			sContCheque.append("@@*p950x2225Y" + sDir2.substring(0, sDir2.indexOf('|')) + " ");
			sContCheque.append("@@*p950x2260Y" + sDir2.substring(sDir2.indexOf('|')+1, sDir2.length()) + " C.P. " + listCheques.get(renglon).getIdCp() + " ");
			sContCheque.append("@@*p950x2295Y" + "R.F.C. " + listCheques.get(renglon).getCiaRfc() + " ");
			...
			//sContCheque.append("@@(s1p1h14v0s+0b4T @@*p260x2680YNO NEGOCIABLE ");
			//sContCheque.append("@@(s1p1h9v0s+0b4T @@*p260x2730YPARA ABONO EN CUENTA DEL BENEFICIARIO ");
			
			//Fecha
			sContCheque.append("@@(s1p1h10v0s+0b4T @@*p1690x2250Y" + sFechaCheque + " ");
			
			//Beneficiario
			//(listCheques.get(renglon).getBeneficiario()).replaceAll(" ", "&#255;")
			sContCheque.append("@@(s1p1h10v0s+0b4T @@*p70x2415Y" + sNomBenef + " ");
			
			//Cantidad en letra
			sContCheque.append("@@(s1p1h10v0s+0b4T @@*p70x2590Y" + "(" + importeLetra + ") ");
			...
			//No Cuenta
			sContCheque.append("@@(s1p1h10v0s+0b4T ");
			sContCheque.append("@@*p970x2840Y" + sNoCta + " ");
			
			//No Sucursal y plaza
			sContCheque.append("@@*p970x2890Y" + listCheques.get(renglon).getSucursal()); // + " " + listCheques.get(renglon).getPlaza() + " ");
			
			//No Cheque
			sContCheque.append("@@*p970x2940Y" + funciones.ponerCeros(""+listCheques.get(renglon).getNoCheque(), 7));
			...
			sContCheque.append(formatoImporte(1935,2410, sAux , 'G'));
			...
			//Firmante 1
			List<MantenimientosDto> lMant = mantenimientosDao.buscaFirmantes(listCheques.get(renglon).getNo_firmante1());
			if(lMant!=null && lMant.size()>0)
				firma1 = lMant.get(0).getPathFirma();
			
			//Firmante 2
			lMant = mantenimientosDao.buscaFirmantes(listCheques.get(renglon).getNo_firmante2());
			if(lMant!=null && lMant.size()>0)
			...
			sContCheque.append("@@*p1420x2890Y" + firma1 + " @@*p1880x2880Y" + firma2 + " @@%-20010X ");
			...
			if(iBcoCve == 21){
				...
				sContCheque.append("@@*p600x3075Y@@(0Q@@(s0p8.00h8 v0s0b0T" + sCerseg + iDigitoPremarcado + "t");
				//sContCheque.append("</p>");
				//sContCheque.append("<p>");
				sContCheque.append("@@*p790x3075Y@@(0Q@@(s0p8.00h8 v0s0b0T" + sTransito + "t");
				//sContCheque.append("</p>");
				//sContCheque.append("<p>");
				sContCheque.append("@@*p1170x3075Y@@(0Q@@(s0p8.00h8 v0s0b0T" + funciones.ponerCeros(listCheques.get(renglon).getNoCuentaS(), 11) + "o");
				//sContCheque.append("</p>");
				//sContCheque.append("<p>");
				sContCheque.append("@@*p1626x3075Y@@(0Q@@(s0p8.00h8 v0s0b0T" + funciones.ponerCeros(""+listCheques.get(renglon).getNoCheque(), 7) + " ");
				//sContCheque.append("</p>");
			...
			//Fin del Cheque
			sContCheque.append("@@(3@ @@%-20011X @@E@@%-12345X ");
	...
	/** ----------------FIN DE RUTINA IMPRESORA HP --------------- **/

	
	/********* NUEVA RUTINA DE IMPRESION DE CHEQUE JRC **********/
		...
		try
		{
			...
			*/
			...
			if(iGBcoFirma != listCheques.get(renglon).getBcoCve() || sGCtaFirma != listCheques.get(renglon).getCtaNo())
			{
				...
				catalogoFirmas = impresionDao.consultarCatalogoFirmas(iGBcoFirma, sGCtaFirma);
	...
	/********* FIN DE NUEVA RUTINA DE IMPRESION DE CHEQUE ********/
...
//			'DIGITO VERIFICADOR DEL NUMERO DE CHEQUE PARA BANCRECER 161
		...
		catch(Exception e)
		{
			...
			bitacora.insertarRegistro(new Date().toString() + " " + Bitacora.getStackTrace(e)
			+ "P:Impresion, C:ImpresionBusinessImpl, M:rutinaChequeLaserDescE330NF");
			return chequeLaser;
		}
		return chequeLaser;
	}
...
//		    'con sus coordenadas y caracteristicas para imprimirlo
		    ...
		    campos = impresionDao.consultarConfiguracionCheques(iIdBanco, sIdDivisa, iNoEmpresa,"");
...
//		    				}
			...
			bitacora.insertarRegistro(new Date().toString() + " " + Bitacora.getStackTrace(e)
			+ "P:Impresion, C:ImpresionBusinessImpl, M:rutinaChequeContinuo");
			return chequeContinuo;
		}
		return chequeContinuo;
	}
	...
	public Map<String, Object>ejecutarProteccionCheque(List<MovimientoDto> listCheques, boolean chkH2H, boolean pbPendientes){
	    ...
	    try{
	    	...
	    	if (listDatosCadena.size()!=0 ) {
	    		...
	    		if (!dirPrincipal.equals("")) {
	    			archProtegido = impresionDao.consultarPropiedadesArchivo(listCheques.get(0).getIdBanco());
	    			sDirectorioDos = archProtegido.get(0).getPathProtegido();
					piConsecutivo = archProtegido.get(0).getNoProtegido() + 1;
			        impresionDao.actualizarCatBanco(listCheques.get(0).getIdBanco());
	    	...
	    	System.out.println("tamaño "+iNreg+"  "+listCheques.get(0).getIdBanco());
	        
	    	for(int i = 0; i < listCheques.size(); i++){
	    		switch (listCheques.get(i).getIdBanco()){
	    			case ConstantesSet.SANTANDER:
	    				/*List<Map<String, Object>> listResultSantander= 
	    						impresionDao.consultarDetalleChaqueAzteca(
	    	    						listCheques.get(i).getNoEmpresa(), listCheques.get(i).getPoHeaders(),
	    	    						listCheques.get(i).getIdBanco(), listCheques.get(i).getIdChequera(),
	    	    						listCheques.get(i).getFecPropuestaStr());*/
	    				...
	    				if(sRegistro!=null ){
	    					...
	    					sPolizas = sPolizas + listCheques.get(i).getPoHeaders() + "','";
	    			...
	    			case ConstantesSet.BANCOMER:
		    					...
		    					sRegistro = layoutProteccionHSBCBusiness.armaBancomerH2HA(obtenerParamsLayout(listCheques, i), 2, 
		    							listCheques.size(), totalImporteArchivo,sArchivo, pbPendientes);
		    					System.out.println(" cadena "+sRegistro);		    					
		    					if(sRegistro!=null && !sRegistro.equals(""))
		    					{
			    					sFolios = sFolios + listCheques.get(i).getNoFolioDet() + ",";
			    					sPolizas = sPolizas + listCheques.get(i).getPoHeaders() + "','";
	    			...
	    			case ConstantesSet.AZTECA:
	    				List<Map<String, Object>> listResult= impresionDao.consultarDetalleChaqueAzteca(
	    						listCheques.get(i).getNoEmpresa(), listCheques.get(i).getPoHeaders(),
	    						listCheques.get(i).getIdBanco(), listCheques.get(i).getIdChequera(),
	    						listCheques.get(i).getFecPropuestaStr());
	    				if (listResult != null && listResult.size()>0) {
							...
							result.put("msgUsuario", "Error al obtener los datos del Layout.");
						}
                    break;
	    		...
	    		poHeaders = listCheques.get(i).getPoHeaders();
	    		noEmpresa = listCheques.get(i).getNoEmpresa();
	    		idBanco = listCheques.get(i).getIdBanco();
	    		idChequera = listCheques.get(i).getIdChequera();
	    		fecPropuesta = listCheques.get(i).getFecPropuestaStr();
	    		...
	    		tipoEnvio = impresionDao.consultarTipoEnvio(sNoDocto, dImporte, iFolios2);
	            
	    		if(tipoEnvio.size() != 0){
	    			for(int j = 0; j < tipoEnvio.size(); j++){
	    				if(tipoEnvio.get(j).getTipoEnvioLayout().equals("CHE")){
	    					//impresionDao.actualizarDetArchTransfer(tipoEnvio.get(j).getNoFolioDet()); //, lsArchivo2);
	    					impresionDao.insertarDetArchTransfer(sArchivo2,  poHeaders, sBenef, dImporte,
	    							fecPropuesta,idChequera, idBanco, sConcepto,noEmpresa, 1);
	    				}else{
	                    	impresionDao.insertarDetArchTransfer(sArchivo2,  poHeaders, sBenef, dImporte,
	                    			fecPropuesta,idChequera, idBanco, sConcepto,noEmpresa,  1);
	    				}
	    	...
	    	if(bBien){
	    		...
	    		impresionDao.insertarArchTransfer(sArchivo2, iNbanco, dFecha, dImporte, iUsuario, iNreg);
	    		
	    		if(!sPolizas.equals("")){
	    			sPolizas = sPolizas.substring(0, sPolizas.length()-2);
	    			impresionDao.actualizarMovimientoHistMovimiento(sArchivo, sPolizas);
	    		}
	    		if (chkH2H){
	    			result.put("msgUsuario", "Se genero el archivo " + dirPrincipal + sArchivo2 + ".");
	    		}else{
	    			System.out.println("\n dir"+ dirPrincipal);
	    			result.put("msgUsuario",  "Los registros han sido guardados en el archivo "  +sArchivo);
		...
		}catch(Exception e)
		{
			e.printStackTrace();
			bitacora.insertarRegistro(new Date().toString() + " " + Bitacora.getStackTrace(e)
			+ "P:Impresion, C:ImpresionBusinessImpl, M:ejecutarProteccionCheque");
		}
		return result;
	}
	 ...
	 * @since 27/06/2011
	 */
	public Map<String, Object>ejecutarProteccionChequeRes(List<MovimientoDto> listCheques, boolean chkH2H, boolean pbPendientes){
		Map<String,Object> result= new HashMap<String,Object>();
		globalSingleton = GlobalSingleton.getInstancia();
		Map<String, Object> mapRespRegMeto = new HashMap<String, Object>();
		mapRespRegMeto.put("msgUsuario","Error desconocido.");
	    ...
	    try{
	    	...
	    	for(int i = 0; i < listCheques.size(); i++)
	    	{
	    		...
	    		case ConstantesSet.SANTANDER:
    				...
    				/*List<Map<String, Object>> listResultSantander= //impresionDao.consultarDetalleChaqueAzteca(String.valueOf(listCheques.get(i).getNoFolioDet()));
    						impresionDao.consultarDetalleChaqueAzteca(
    	    						listCheques.get(i).getNoEmpresa(), listCheques.get(i).getPoHeaders(),
    	    						listCheques.get(i).getIdBanco(), listCheques.get(i).getIdChequera(),
    	    						listCheques.get(i).getFecPropuestaStr());*/
                ...
                break;
	    			...
	    			case ConstantesSet.BANCOMER:
		    					...
		    					sRegistro = layoutProteccionHSBCBusiness.armaBancomerH2HA(obtenerParamsLayout(listCheques, i), 1, 
		    							listCheques.size(), totalImporteArchivo, sArchivo, pbPendientes);
		    							    					
		    					if(sRegistro!=null && !sRegistro.equals("")){
		    						result = creacionArchivosBusiness.escribirArchivoLayout(dirPrincipal,sDirectorioDos,sArchivo,sRegistro);
		    						if(!(boolean)result.get("resExito")){
		    							result.put("msgUsuario","Error al generar el archivo de proteccion de Bancomer!");
		    							bBien = false;
	    			 ...
	    			 break;
	    			case ConstantesSet.AZTECA:
	    				List<Map<String, Object>> listResult= //impresionDao.consultarDetalleChaqueAzteca(""+listCheques.get(i).getNoFolioDet());
	    						impresionDao.consultarDetalleChaqueAzteca(
	    	    						listCheques.get(i).getNoEmpresa(), listCheques.get(i).getPoHeaders(),
	    	    						listCheques.get(i).getIdBanco(), listCheques.get(i).getIdChequera(),
	    	    						listCheques.get(i).getFecPropuestaStr());
	    		...
	    		if(tipoEnvio.size() != 0){
	    			for(int j = 0; j < tipoEnvio.size(); j++){
	    				if(tipoEnvio.get(j).getTipoEnvioLayout().equals("CHE")){
	    					impresionDao.actualizarDetArchTransfer(tipoEnvio.get(j).getNoFolioDet()); //, lsArchivo2);
	    					//impresionDao.insertarDetArchTransfer(sArchivo2, sBenef, dImporte, iFolios2, dFecha, sChe2, iNbanco, sConcepto, sNoDocto, 1);
	    				}
	                  //  else
	                    	//impresionDao.insertarDetArchTransfer(sArchivo2, sBenef, dImporte, iFolios2, dFecha, sChe2, iNbanco, sConcepto, sNoDocto, 1);
	    			}
	    	...
	    	if(bBien)
	    	{
	    		...
	    		if(!sFolios.equals(""))
	    		{
	    			...
	    			impresionDao.actualizarMovimientoHistMovimiento(sArchivo, sFolios);
	    		}
	    		if (chkH2H)
	    		{
	    			result.put("msgUsuario", "Se genero el archivo " + dirPrincipal + sArchivo2 + ".");
	...
	public LayoutProteccionDto obtenerParamsLayout(List<MovimientoDto> listCheques, int i){
		...
		try{
			...
			dto.setFecPropuesta(listCheques.get(i).getFecPropuestaStr());
			dto.setConcepto(listCheques.get(i).getConcepto());
	
		}catch(Exception e){
			bitacora.insertarRegistro(new Date().toString() + " " + Bitacora.getStackTrace(e)
			+ "P:Impresion, C:ImpresionBusinessImpl, M:obtenerParamsLayout");
		}
		return dto;
	}
	...
	public List<MovimientoDto>consultarSolicitudesReimpresion(ConsultaChequesDto dtoCheques){
		...
		list = impresionDao.consultarSolicitudesReimpresion(dtoCheques);
		return list;
	}
	...
	public Map<String, Object>ejecutarReimpresionCheques(
		...
		try{
			...
			if (!bOptImpresionContinua) {
	    		...
	    		List<LlenaComboImpresora> listCharolas = 
	    			impresionDao.consultarCharolasImpresora(
	    					iNoImpresora, iCaja);
		        ...
		        List<CatCtaBancoDto> listContinuo = 
		        	impresionDao.consultarImpresionContinua(
		        			iBanco, sChequera);
		        ...
		        //EMS: 29/01/2016 - Se agregó un campo adicional para buscar por configuración, esta funcion ya no tiene utilidad sin el id.
		        List<ConfiguracionChequeDto> listConfigurados = 
		        	impresionDao.consultarConfiguracionCheques(iBanco, sMoneda, iEmpresa, "");
		    ...
		    for (int i = 0; i < listCheques.size(); i++) {
		    	List<MovimientoDto> entregaCheque = 
		    		impresionDao.consultarEntregaCheque(
			...
			e.printStackTrace();
			bitacora.insertarRegistro(new Date().toString() + " " + Bitacora.getStackTrace(e)
			+ "P:Impresion, C:ImpresionBusinessImpl, M:ejecutarReimpresionCheques");
		}
		return retorno;
	}
	...
	public void obtenerReporteCheque(Map parameters){
		...
		try
		{
			...
			List<GuiaContableDto> lCtaContable = impresionDao.obtenerCtaContable(sCuenta);
			
			/*List<GuiaContableDto> lCtaContable = impresionDao.consultarCtaContableRubro(noEmpresa);
			...
			List<CatEquivaleCuentaContableDto> lEquivalencia = impresionDao.consultarEquivalenciaCtaContable(noEmpresa, sCuenta);
			if(lEquivalencia.size() == 0)
			{
			...
			*/
			...
			impresionDao.actualizaCheque(noCheque, sCuenta);
			
			//jrDataSource = new JRMapArrayDataSource(resp.toArray());
		}
		catch(Exception e) {
			e.printStackTrace();
			bitacora.insertarRegistro(new Date().toString() + " " + Bitacora.getStackTrace(e) + "P:ImpresionCheques, C:ImpresionBusinessImpl, M:obtenerReporteCheque");
	...
	public String escribirExcelCoorporativo(String datosGrid) {
		...
		*/
		...
		try {
			...
			List<GuiaContableDto> lCtaContable = impresionDao.obtenerCtaContable(idChequera);
			...
			/*List<GuiaContableDto> lCtaContable =
				impresionDao.esAgrupado(noChequeAnt) ?
						impresionDao.
							consultarCtaContableRubroAgrupado(
								noChequeAnt)
						:
						impresionDao.consultarCtaContableRubro(
							Integer.parseInt(
								parameters.get(0).get(
									"noFolioDet").toString()))
						;
            ...
            */
            ...
            if (impresionDao.esAgrupado(noChequeAnt)) {
...
/*            File archivoXLS = new File(nomArch);
  ...
  */          
	...
	public int obtieneUltimoCheqImp(String chequera) {
		return impresionDao.obtieneUltimoCheqImp(chequera);
	}
	...
	public int getUltimoImpCtrlCheque(String noEmpresa, String noBanco, String noChequera) {
		...
		try{
			count = impresionDao.getUltimoImpCtrlCheque(noEmpresa, noBanco, noChequera);
		}catch(Exception e){
			bitacora.insertarRegistro(new Date().toString() + " " + Bitacora.getStackTrace(e) 
	...
	public String creaRutaCheques(int noEmpresa) {
		...
		try {
			...
			List<LayoutProteccionDto> fechaHoy = 
				impresionDao.obtieneFechaHoy();
			
			String nomEmpresa = 
				impresionDao.obtieneNomCarpeta(noEmpresa);
			String mes = 
				funciones.nombreMes(
			...
			rutaCheque = 
				impresionDao.consultarConfiguraSet(3007) + 
				nomEmpresa + "\\" + mes + year + "\\";
		...
		*/			
		...
		try {
            ...
            */
            ...
            respUpdate = impresionDao.actualizaCheque(
            		noCheque, 
            		parameters.get(0).get("ctaAlias").toString());
	...
	public List<LlenaComboGralDto>llenarComboBeneficiario(LlenaComboGralDto dto){
		return impresionDao.llenarComboBeneficiario(dto);	
	}

	@Override
	public List<LlenaComboGralDto> llenarComboLeyenda() {
		return impresionDao.llenarComboLeyenda();
	}
	
	public List<LlenaComboGralDto> llenarComboConfiguracion() { //YEC 28.01.16
		return impresionDao.llenarComboConfiguracion();
	}
	...
	public List<LlenaComboGralDto> llenarComboBancos(int noEmpresa) {
		return impresionDao.llenarComboBancos(noEmpresa);
	}

	@Override
	public List<LlenaComboGralDto> llenarComboBancosCC(int idBanco) {
		return impresionDao.llenarComboBancosCC(idBanco);
	}
	
	@Override
	public List<LlenaComboGralDto> llenarComboChequera(int idBanco, int noEmpresa) {
		return impresionDao.llenarComboChequera(idBanco, noEmpresa);
	}
	...
	public String accionarLeyEmpresa(int noEmpresa) {
		String result = "";
		try {
			result = impresionDao.obtenerNombreEmpresa(noEmpresa);
		} catch (Exception e) {
			bitacora.insertarRegistro(new Date().toString() + " " + Bitacora.getStackTrace(e) +
					"P:ImpresionCheques, C:ImpresionBusinessImpl, M:accionarLeyEmpresa");
		}
		return result;
	}
