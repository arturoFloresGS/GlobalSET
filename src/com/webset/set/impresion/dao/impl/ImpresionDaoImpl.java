...
public class ImpresionDaoImpl implements ImpresionDao{
	...
	private static Logger logger = Logger.getLogger(ImpresionDaoImpl.class);
...
//				'    sql.append("\n        firmante2,"
			...
			sql.append("\n        LEFT JOIN cat_caja cc ON (m.id_caja = cc.id_caja)");
				...
				//Posible cambio de condicion a fecha between fec_hoy y fec_ma�ana, dependiendo el critero del radio.
	...
	public List<MovimientoDto>consultarChequesPendientes2(ConsultaChequesDto dtoCheques){
		...
		try
		{
			sql.append("\n SELECT (select desc_caja from cat_caja where id_caja = m.id_caja) AS desc_caja,m.no_empresa, m.po_headers as no_solicitud, ");
			//sql.append("\n        m.importe,");
			sql.append("\n        sum(m.IMPORTE) as importe, m.no_cliente, m.beneficiario, cb.desc_banco,");
			sql.append("\n        m.id_chequera  as cta_alias, (select desc_caja from cat_caja where id_caja = m.id_caja) desc_caja, m.concepto, "); //m.no_factura AS concepto, ");
			sql.append("\n		  e.nom_empresa as cia_nmbr, 0 as centro_costo, m.id_banco as bco_cve, ");
			sql.append("\n        m.id_tipo_movto as tipo_egreso, sum(m.IMPORTE_ORIGINAL) as importe_original, ");
			//sql.append("\n        m.importe_original as importe_original, ");
			sql.append("\n        m.no_docto as no_contrarecibo, (select rfc from persona where id_tipo_persona = 'E' and no_empresa = m.no_empresa) as rfc, m.id_divisa as moneda,");
			sql.append("\n        ' ' as id_contable, m.id_chequera as cta_no, ' ' as leyenda_prote, ");
			sql.append("\n        ccb.desc_plaza as plaza, ccb.desc_sucursal as sucursal, cb.b_protegido,");
			sql.append("\n        m.id_chequera as no_cuenta, cb.codigo_seguridad, cb.cve_plaza as cve_plaza,");
			sql.append("\n        cb.id_banco as no_banco,");
//			sql.append("\n        dir.calle_no + '|' + dir.colonia as cia_dir, ");
//			sql.append("\n        dir.deleg_municipio + '|' + dir.ciudad as cia_dir2, ");
			sql.append("\n        (select d.calle_no + '|' + d.colonia");
			sql.append("\n         from direccion d");
			sql.append("\n         where d.no_persona = (select ccb_h.no_empresa");
			sql.append("\n                               from cat_cta_banco ccb_h");
			...
			sql.append("\n               and d.id_tipo_direccion='OFNA'");
			sql.append("\n        ) as cia_dir2,");
			sql.append("\n        (select rfc from persona where id_tipo_persona = 'E' and no_empresa = m.no_empresa) as cia_rfc, ");
			sql.append("\n        m.no_cheque as no_cheque,");
			sql.append("\n (SELECT RTRIM(RTRIM(LTRIM(cat.nombre)) + ' ' + RTRIM(LTRIM(ISNULL(cat.paterno,''))) + ' ' + RTRIM(LTRIM(ISNULL(cat.materno,''))))");
            sql.append("\n    FROM cat_usuario cat");
            sql.append("\n    WHERE cat.no_usuario=m.firmante1) as firmante1,");
            sql.append("\n (SELECT RTRIM(RTRIM(LTRIM(cat.nombre)) + ' ' +RTRIM(LTRIM(ISNULL(cat.paterno,''))) + ' ' + RTRIM(LTRIM(ISNULL(cat.materno,''))))");
            sql.append("\n    FROM cat_usuario cat");
            sql.append("\n    WHERE cat.no_usuario=m.firmante2) as firmante2,");
            sql.append("\n m.firmante1 as noFirmante1, m.firmante2 as noFirmante2,");
			sql.append("\n        m.PO_HEADERS,");
			...
			sql.append("\n FROM   movimiento m LEFT JOIN cat_banco cb ON (m.id_banco = cb.id_banco)");
			sql.append("\n        LEFT JOIN cat_caja cc ON (m.id_caja = cc.id_caja) ");
			sql.append("\n        LEFT JOIN direccion dir ON (m.no_empresa = dir.no_empresa AND m.no_empresa = dir.no_persona AND dir.id_tipo_persona = 'E')");
	            //sbSQL.append("\n   AND d.id_tipo_persona = 'E') ");
				...
				//Se quitara la forma de pago 7 y 10 
				...
				sql.append("\n m.c_periodo, m.no_folio_mov, firmante1, firmante2, m.concepto, m.no_docto ");
				...
				listRetorno = jdbcTemplate.query(sql.toString(), new RowMapper<MovimientoDto>(){
					public MovimientoDto mapRow(ResultSet rs, int idx) throws SQLException {
						...
						movimiento.setFirmante1(rs.getString("firmante1") == null ? "" : rs.getString("firmante1"));
						movimiento.setFirmante2(rs.getString("firmante2") == null ? "" : rs.getString("firmante2"));
						movimiento.setNo_firmante1(rs.getInt("noFirmante1"));
						movimiento.setNo_firmante2(rs.getInt("noFirmante2"));
						movimiento.setFecCheque(rs.getDate("fec_cheque"));
						
						if(rs.getDate("fec_cheque") != null)
							movimiento.setFecChequeStr(funciones.ponerFechaSola(rs.getDate("fec_cheque"))); //Se obtiene para efectos de visualizacion, fecCehque de tipo Date ya estaba.
	...
	public List<LlenaComboImpresora>llenarComboImpresora(int caja)
	{
		...
		try
		{
			...
			sql.append( "       conver(numeric,CJA_CVE) as CJA_CVE,");
	...
	public List<LlenaComboGralDto> llenarComboBancosCC(int idBanco){
		...
		try{
			...
			e.printStackTrace();
			bitacora.insertarRegistro(new Date().toString() + " " + Bitacora.getStackTrace(e)
			+ "P:Impresion, C:ImpresionDaoImpl, M:llenarComboBancosCC");
		}
		return list;
	...
	public List<LlenaComboGralDto>llenarComboChequera(int idBanco, int noEmpresa) {
		...
		try {
			...
			bitacora.insertarRegistro(new Date().toString() + " " + Bitacora.getStackTrace(e)
		    + "P:Impresion, C: ImpresionDaoImpl, M: llenarComboChequera");
		}return list;
	}
	...
	public List<LlenaComboImpresora>consultarCharolasImpresora(int noImpresora, int caja)
	{
		...
		try
		{
            sql.append("SELECT convert(numeric,num_charolas) as num_charola, tipo_impresora ");
...
//		    'sql.append("\n     OR  folio_ult_impreso = folio_inv_ini - 1)");
	...
	//Agregado EMS 30/12/2015 - Se agreg� la chequera, por el momento no se busca por chequera cuando es folio_papel. 
	//si retorna m�s de un dato, en business se tomar� el �ltimo que se registro.
	...
	public int actualizarMovimiento(Date fechaHoy, int usuario, Date fechaChequeCapturada, Date fechaCheque,
		...
		try{
			...
			sql.append("\n  AND NO_EMPRESA = "+noEmpresa+" ");
			sql.append("\n  AND FEC_PROPUESTA = convert(date,'"+Utilerias.validarCadenaSQL(fechaPropuesta)+"',103) ");
			sql.append("\n  AND (C_PERIODO = "+periodo+" OR C_PERIODO IS NULL) ");
	...
	public int insertarBitacoraCheques(InsertaBitacoraChequesDto dto){
	    ...
	    sql.append("\n             , " + dto.getImporte() + ", '" + Utilerias.validarCadenaSQL(dto.getEstatus())+ "', '" + Utilerias.validarCadenaSQL(dto.getDivisa()) + "', GETDATE()");
	...
	public int actualizarCaja(int caja, int noFolioMov){
		...
		try{
			...
			e.printStackTrace();
			bitacora.insertarRegistro(new Date().toString() + " " + Bitacora.getStackTrace(e) 
			+ "P:Impresion, C:ImpresionDaoImpl, M:actualizarCaja");
		}
		return updateCaja;
	}
	 ...
	 */
	//Modificado A.A.G
	public List<MovimientoDto>consultarDocumentosChequesRespaldo(ConsultaChequesDto dtoCheques){
		List<MovimientoDto>listRetorno = new ArrayList<MovimientoDto>();
		...
		try{
		    ...
		    if(dtoCheques.getCriterioFechaIni() != null && !dtoCheques.getCriterioFechaIni().equals("")){
		    	if(dtoCheques.getCriterioFechaFin() != null && !dtoCheques.getCriterioFechaFin().equals("")){
			        sSQL.append("\n        AND m.fec_imprime >= convert(datetime,'" + Utilerias.validarCadenaSQL(funciones.ponerFechaSola(dtoCheques.getCriterioFechaIni())) +"',103)"+
                                "          AND m.fec_imprime <= convert(datetime,'" + Utilerias.validarCadenaSQL(funciones.ponerFechaSola(dtoCheques.getCriterioFechaFin()))+"',103)");
				}else
			        sSQL.append("\n        AND m.fec_imprime = convert(datetime,'" + Utilerias.validarCadenaSQL(funciones.ponerFechaSola(dtoCheques.getCriterioFechaIni())) +"',103)");
			}
	...
	public List<MovimientoDto>consultarDocumentosCheques(ConsultaChequesDto dtoCheques){
		...
		try{
			...
			sSQL.append("\n ,M.Id_Divisa");
			sSQL.append("\n ,m.no_cheque");
			sSQL.append("\n ,convert(char(10),M.Fec_Propuesta, 103) as Fec_Propuesta");
			sSQL.append("\n,'PAGO CHEQUE' as concepto"); 
			sSQL.append("\n From Movimiento M Join Cat_Banco C On (");
			sSQL.append("\n M.Id_Banco = C.Id_Banco)");
			sSQL.append("\n Join Cat_Cta_Banco Ccb On (");
			sSQL.append("\n M.No_Empresa = Ccb.No_Empresa And");
			sSQL.append("\n M.Id_Banco = Ccb.Id_Banco And ");
			sSQL.append("\n m.id_chequera = ccb.id_chequera )");
	        
			sSQL.append("\n WHERE ");
			sSQL.append("\n (M.Po_Headers is not null or M.Po_Headers <> ''  or M.Po_Headers <> ' ')");
			sSQL.append("\n AND id_forma_pago in (1,7,10) ");
			sSQL.append("\n AND id_tipo_operacion in(3200)");
		  ...
		  //  sSQL.append("\n      OR id_estatus_cb in ('P'))");
		    ...
		    sSQL.append("\n Group By m.no_empresa,M.Po_Headers, M.Id_Banco, M.Id_Chequera, M.Beneficiario,");
    		sSQL.append("\n fec_imprime, c.desc_banco, ccb.desc_plaza , m.id_divisa, m.no_cheque, m.fec_propuesta ");
			...
			sSQL.append("\n ,convert(char(10),M.Fec_Propuesta, 103) as Fec_Propuesta");
			sSQL.append("\n,'PAGO CHEQUE' as concepto"); 
			sSQL.append("\n From hIST_Movimiento M Join Cat_Banco C On (");
			sSQL.append("\n M.Id_Banco = C.Id_Banco)");
			sSQL.append("\n Join Cat_Cta_Banco Ccb On (");
			sSQL.append("\n M.No_Empresa = Ccb.No_Empresa And");
			sSQL.append("\n M.Id_Banco = Ccb.Id_Banco And ");
		  ...
		  //  sSQL.append("\n      OR id_estatus_cb in ('P'))");
			...
			listRetorno = jdbcTemplate.query(sSQL.toString(), new RowMapper<MovimientoDto>(){
				public MovimientoDto mapRow(ResultSet rs, int idx) throws SQLException {
					...
					movimiento.setIdChequera(rs.getString("id_chequera"));
					movimiento.setNoCheque(rs.getInt("no_cheque"));
					movimiento.setNoFolioDet(0);
					movimiento.setPoHeaders(rs.getString("po_headers"));
					movimiento.setBeneficiario(rs.getString("beneficiario"));
					movimiento.setImporte(rs.getDouble("importe"));
					movimiento.setFecImprime(rs.getDate("fec_imprime"));
					movimiento.setFecPropuestaStr(rs.getString("Fec_Propuesta"));
					movimiento.setDescBanco(rs.getString("desc_banco"));
					movimiento.setConcepto(rs.getString("concepto"));
					movimiento.setPlaza(rs.getInt("plaza"));
					movimiento.setIdDivisa(rs.getString("id_divisa"));
					movimiento.setNoDocto(rs.getString("Po_Headers"));
					movimiento.setNoEmpresa(rs.getInt("no_empresa"));
					return movimiento;
				}
			});
			...
			e.printStackTrace();
			bitacora.insertarRegistro(new Date().toString() + " " + Bitacora.getStackTrace(e)  
			+ "P:Impresion, C:ImpresionDaoImpl, M:consultarDocumentosChequesAgrupados");
		}
		return listRetorno;
	}
	 ...
	 * @return
	 */
	public int insertarDetArchTransfeRES(String sArchivo2, String sBenef, double dImporte, int sFolios, Date dFecha,
	...
	public int insertarDetArchTransfer(String sArchivo2, String poHeaders,String sBenef, double dImporte,
		...
		StringBuffer sSQL = new StringBuffer();
		try{
		    sSQL.append("\n INSERT INTO det_Arch_transfer_agrup");
		    sSQL.append("\n (nom_arch, po_headers, beneficiario, ");
    		sSQL.append("\n importe, fec_propuesta, id_chequera,");
    		sSQL.append("\n id_banco,concepto,id_estatus_arch,id_chequera_benef,no_empresa)");
		        
		    sSQL.append("\n values (");
		    sSQL.append("\n '" + Utilerias.validarCadenaSQL(sArchivo2)+ "','" + Utilerias.validarCadenaSQL(poHeaders) + "','" +
		    					Utilerias.validarCadenaSQL(sBenef) + "'," + dImporte + ", ");
		    sSQL.append("\n" + "convert(datetime,'"+ Utilerias.validarCadenaSQL(fecPropuesta)  
		    				 + "',103),'" + Utilerias.validarCadenaSQL(idChequera) + "',");
		    sSQL.append("\n" +  idBanco + ",'" + Utilerias.validarCadenaSQL(sConcepto) 
		    				 +  "','T'," +  iCheque +","+noEmpresa + ")");
		     
		    //logger.info("inserta detarchtransfer");
		    System.out.println("este es el incert  "+sSQL);
		    inserta = jdbcTemplate.update(sSQL.toString());
		}catch(Exception e){
			e.printStackTrace();
			bitacora.insertarRegistro(new Date().toString() + " " + Bitacora.getStackTrace(e)  
	...
	public List<Map<String, Object>> consultarDetalleChaqueAzteca(int noEmpresa, String poHeaders,
		 ...
		 try {
			...
			sSQL.append("\n Where M.Po_Headers = '"+poHeaders+"'");
			sSQL.append("\n  And M.No_Empresa = "+noEmpresa);
			sSQL.append("\n And M.Id_Banco = "+idBanco+"");
			sSQL.append("\n And M.Id_Chequera = '"+idChequera+"'");
			//sSQL.append("\n and M.Fec_Propuesta = '"+fecPropuesta+"'");
			sSQL.append("\n AND FEC_PROPUESTA = convert(datetime,'"+fecPropuesta.toString()+"',103) ");
			sSQL.append("\n UNION ALL Select E.Nom_Empresa,p.rfc, m.solicita");
	...
	public List<Map<String, Object>> consultarDetalleChaqueAztecaRespaldo(String noFolioDet){
		 List<Map<String, Object>> listResult = new ArrayList<Map<String, Object>>();
		 StringBuffer sSQL = new StringBuffer();
		 try {
			sSQL.append("");
	...
	public int actualizarMovimientoHistMovimientoRes(String sArchProtegido, String sFolios){
		int actualiza = -1;
		StringBuffer sSQL = new StringBuffer();
		try{
			sSQL.append("\n UPDATE movimiento " );
	...
	public List<MovimientoDto>consultarSolicitudesReimpresion(ConsultaChequesDto dtoCheques){
		...
		try{
			...
			//if(ConstantesSet.gsDBM.equals("ORACLE")){
	            ...
	            sbSQL.append("\n        cat_cta_banco ccb, empresa e");            //'se agreg� la tabla movimiento
	            //sbSQL.append("\n        direccion d, cat_cta_banco ccb, empresa e");            //'se agreg� la tabla movimiento
			...
			if(ConstantesSet.gsDBM.equals("SQL SERVER"))
			{
	            ...
	            if(dtoCheques.getChequeNomina() == 1) 			//Se agrega if para determinar si el usuario puede ver los cheques de n�mina o gastos
	            	sbSQL.append("\n   AND m.id_area = 1" );	//'Visualiza s�lo cheques de n�mina
                else if(dtoCheques.getChequeNomina() == 2)
                	sbSQL.append("\n   AND m.id_area = 2"); 	//'Visualiza s�lo cheques de gastos
	            ...
	            if(dtoCheques.getChequeNomina() == 1) 			//Se agrega if para determinar si el usuario puede ver los cheques de n�mina o gastos
	            	...
	            	sbSQL.append("\n   AND m.id_area IS NOT NULL");	//'Visualiza s�lo cheques donde id_area IS NULL
	...
	public List<MovimientoDto>consultarEntregaCheque(String folio){
		...
		try{
		    ...
		    sbSQL.append("\n   FROM movimiento ");
		    sbSQL.append("\n  WHERE b_entregado = 'S' ");
		    sbSQL.append("\n    AND no_folio_det = '"+Utilerias.validarCadenaSQL(folio)+"' ");
	...
	*/
	...
	{	
		...
		try
		{
			sql += "Select dateadd(month, 3, fec_hoy) as fec_venc from fechas ";
									
			lista  = jdbcTemplate.query(sql, new RowMapper<Date>() 
			{
				public Date mapRow(ResultSet rs, int idx) throws SQLException 
				{
					Date fecha = new Date();					
					fecha = (rs.getDate("fec_venc"));										
					return fecha;			
				}
			});
	...
	//Metodo agregado EMS 15/12/2015
		...
		//Agregado EMS 19/01/16 - Verifica que exista otra chequera en estatus I para la impresi�n
	 ...
	 * @Objetivo: Obtener de movimiento los firmantes 1 y 2 para efectos de impresi�n.
	 */
