...
public class CompensarPropuestasAction {
	...
	public List<LlenaComboGralDto> llenarComboGrupoEmpresas(int idUsuario){
		...
		try{
			if(Utilerias.haveSession(WebContextManager.get()) && Utilerias.tienePermiso(WebContextManager.get(),204)){
				CompensarPropuestasService CompensarPropuestasService = (CompensarPropuestasService)contexto.obtenerBean("compensarPropuestasBusinessImpl");
				dtoIn.setIdUsuario("" + idUsuario);
				listRet=CompensarPropuestasService.llenarComboGrupoEmpresa(dtoIn);
			}
		}catch(Exception e){
	...
	public List<LlenaComboGralDto> llenarComboDivXEmp(int idUsuario){
		...
		try{
			if(Utilerias.haveSession(WebContextManager.get()) && Utilerias.tienePermiso(WebContextManager.get(),204)){
				CompensarPropuestasService CompensarPropuestasService =(CompensarPropuestasService)contexto.obtenerBean("compensarPropuestasBusinessImpl");
				listRet=CompensarPropuestasService.llenarComboDivXEmp(idUsuario);
			}
		}catch(Exception e){
			bitacora.insertarRegistro(new Date().toString() + " " + Bitacora.getStackTrace(e)
	...
	public SaldosChequerasDto obtenerSaldosChequeras(int idGrupoEmpresa, 
		...
		try{
			if(Utilerias.haveSession(WebContextManager.get()) && Utilerias.tienePermiso(WebContextManager.get(),204)){
				...
				dto=CompensarPropuestasService.obtenerSaldosChequeras(dto);
			}
		}
		catch(Exception e){
			bitacora.insertarRegistro(new Date().toString() + " " + Bitacora.getStackTrace(e)
	...
	public List<MovimientoDto> consultarPropuestas(int idGrupoEmpresa, 
		...
		try{
			if(Utilerias.haveSession(WebContextManager.get()) && Utilerias.tienePermiso(WebContextManager.get(),204)){
				...
				listCons = CompensarPropuestasService.consultarPropuestas(dtoIn);
				/*
	...
	public List<ComunEgresosDto> validarCpaVtaTransfer(String datos){
		...
		try{
			if(Utilerias.haveSession(WebContextManager.get()) && Utilerias.tienePermiso(WebContextManager.get(),204)){
				...
				listRetorno = CompensarPropuestasService.validarCpaVtaTransfer(listGrid);
			}
		}catch(Exception e){
			e.printStackTrace();
			bitacora.insertarRegistro(new Date().toString() + " " + Bitacora.getStackTrace(e)
	...
	public Map<String, Object> insetarZexpFact (String datos, String fecHoy, 
		...
		try{
			if(Utilerias.haveSession(WebContextManager.get()) && Utilerias.tienePermiso(WebContextManager.get(),204)){
				CompensarPropuestasService CompensarPropuestasService = (CompensarPropuestasService)
						contexto.obtenerBean("compensarPropuestasBusinessImpl");
				mapRetorno= CompensarPropuestasService.insertarFoliosZexpFact(datos, fecHoy, idUsuario);
			}
		}catch(Exception e){
			bitacora.insertarRegistro(new Date().toString() + " " + Bitacora.getStackTrace(e)
	...
	public List<MovimientoDto>consultarDetalle(int idGrupoEmpresa, int idGrupo, String cveControl, int usr1, int usr2, int usr3){
		...
		try{
			...
			listDetalle = CompensarPropuestasService.consultarDetalle(dtoIn);
		}catch(Exception e){
			bitacora.insertarRegistro(new Date().toString() + " " + Bitacora.getStackTrace(e)
	...
	public List<MovimientoDto>consultarDetallePropuestasNoPagadas(int idGrupoEmpresa, int idGrupo, String cveControl, int usr1, int usr2, int usr3){
		...
		try{
			...
			dtoIn.setUsuarioDos(usr2);
			dtoIn.setUsuarioTres(usr3);
			listDetalle = CompensarPropuestasService.consultarDetallePropuestasNoPagadas(dtoIn);
		}catch(Exception e){
			bitacora.insertarRegistro(new Date().toString() + " " + Bitacora.getStackTrace(e)
			+"P:Egresos, C:ConsultaPropuestasAction, M:consultarDetalle");	
		}
	...
	public Map<String,Object> ejecutarPropuestas(String datos, String fecHoy){
		...
		try{
			if(Utilerias.haveSession(WebContextManager.get()) && Utilerias.tienePermiso(WebContextManager.get(),204)){
				...
				mapRetorno=CompensarPropuestasService.ejecutarPropuestas(listGrid, fecHoy);
			}
		}catch(Exception e){
			bitacora.insertarRegistro(new Date().toString() + " " + Bitacora.getStackTrace(e)
	...
	public HSSFWorkbook consultaPagosExcel(String clave, ServletContext context){
		...
		try {
			...
			CompensarPropuestasService CompensarPropuestasService = (CompensarPropuestasService)contexto.obtenerBean("compensarPropuestasBusinessImpl",context);
			...
			wb=CompensarPropuestasService.consultaPagosExcel(clave);
		} catch (Exception e) {
	...
	public JRDataSource obtenerReportePagoPropuestas(Map datos, ServletContext context) {
		...
		try {
			...
			jrDataSource = CompensarPropuestasService.reportePagoPropuestas(dtoIn);
