...
public class PagosEnSAPDaoImpl implements PagosEnSAPDao{
...
public List<BloqueoPagoCruzadoDto> obtenerBloqueoSAP(String cveControl, String noDocto) {
		...
		try{
		  ...
		  //fin parte del codigo
		   ...
		   //     sql+= " LEFT JOIN ctas_banco cbp ON ( cbp.no_persona = convert(int, m.no_cliente) ";
		    ...
		    sql+= "\n    and (m.comentario1 is null or m.comentario1 = '' or m.comentario1 = ' ')";
	...
	public List<String> consultarPagoMassPayment(int noFolioDet, Date fechaHoy){
		...
		try{
		    ...
		    sql+="\n   AND m.id_forma_pago = 3";
		    sql+="\n   AND m.fec_valor > convert(datetime,'" +Utilerias.validarCadenaSQL(funciones.ponerFecha(fechaHoy))+ "',103)";
	...
	public int actualizarCveControl(int noEmpresa, String psFolios, String cveControl, Date fecPropuesta){
		...
		try{
		    ...
		    sql+="\n       fec_propuesta = convert(char(10),'" +funciones.ponerFecha(fecPropuesta)+ "' ,103)+' '+convert(char(8),'" +funciones.ponerFecha(fecPropuesta)+ "' ,108) ";
	...
	public int inserPagosZexpFact (MovimientoDto folios , int idUsuario){
		...
		try {
			...
			sqlInsert.append("\n convert(char(11),m.fec_propuesta,103) as Fechap,0 as Cheque_Trn,M.Importe as Imp_Usa,3200 as Tipo_Operacion,'E' as Tipo_Movto,0.0 as capital,0.0 as interes,0.0 as isr,M.Id_Chequera,0 as Casa_Bolsa,");
	...
	public DT_Pagos_OBPagos pagosParaWebservice (MovimientoDto folios, boolean bFactoraje, int factoraje){
		...
		try {
			if(folios.getCveControl()!=null && !folios.getCveControl().equals("")){
				...
				sql.append("\n  convert(char(11),m.fec_propuesta,103) As Fechap,0 As Cheque_Trn,");
...
//		    sql+= "\n    AND m.fec_propuesta = '"+funciones.ponerFecha(dtoIn.getFechaPago())+"'";
	...
	public int updateCancelSesionOperacionesCompraDeTransferParaPago( String folios ) {
		...
		try {
			...
			e.printStackTrace();
			bitacora.insertarRegistro(new Date().toString() + " " + Bitacora.getStackTrace(e)
			+ "P:Egresos, C:PagosEnSAPDaoImpl, M:updateCancelSesionOperacionesCompraDeTransferParaPago");
			return 0;
	...
	public void actualizaMovimientoCompensar(List<ComunEgresosDto> listGrid) {
		...
		try {
			for (int i = 0; i < listGrid.size(); i++) {
				...
				sb.append("\n  COMENTARIO1 = null");
				sb.append("\n  where");
				sb.append("\n  cve_control = '"+listGrid.get(i).getCveControl()+"'");
			...
			e.printStackTrace();
			bitacora.insertarRegistro(new Date().toString() + " " + Bitacora.getStackTrace(e)
			+ "P:Egresos, C:PagosEnSAPDaoImpl, M:actualizaMovimientoCompensar");
