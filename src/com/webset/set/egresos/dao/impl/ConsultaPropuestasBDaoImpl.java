...
import com.webset.set.egresos.dto.ConsultaPropuestasBDto;
...
public class ConsultaPropuestasBDaoImpl implements ConsultaPropuestasBDao{
	...
	//Metodo modificado para Oracle: EMS 14/09/15
	public List<SeleccionAutomaticaGrupoDto> consultarSeleccionAutomatica(ConsultaPropuestasDto dtoIn){
		...
		try{
            ...
            sql.append("\n       coalesce((select clave_usuario from seg_usuario where id_usuario = sa.usuario_dos), '') as user2, sa.usuario_tres, coalesce((select clave_usuario from seg_usuario where id_usuario = sa.usuario_tres), '') as user3, f.nivel_autorizacion, sa.id_division, convert (datetime,sa.fecha_pago, 103) as fecha_pago, ");
            sql.append("\n       ( SELECT count(*) ");
            sql.append("\n         FROM movimiento m ");
            ...
            sql.append("\n          END ");
            sql.append("\n FROM CAT_FORMA_PAGO ");
            sql.append("\n WHERE (convert(varchar,ID_FORMA_PAGO)) = substring(sa.concepto, LEN(sa.concepto), LEN(sa.concepto)) ");
            sql.append("\n  ) AS FORMA_PAGO ");
           ...
           if(dtoIn.getIdCliente()!="" && dtoIn.getIdCliente()!=null)
           {
	        ...
	        if(!dtoIn.isTodasPropuestas()){
		        if((dtoIn.getFechaFin()!=null && !dtoIn.getFechaFin().trim().equals("")) && (dtoIn.getFechaIni()!=null && !dtoIn.getFechaIni().trim().equals("")))
		            sql.append( "\n AND sa.fecha_pago between convert(datetime,'"+Utilerias.validarCadenaSQL(dtoIn.getFechaIni())+"', 103)  AND convert(datetime,'"+Utilerias.validarCadenaSQL(dtoIn.getFechaFin())+"',103) ");
		        else if(dtoIn.getFechaIni()!=null && !dtoIn.getFechaIni().trim().equals(""))
		            sql.append( "\n AND sa.fecha_pago >= convert(datetime,'"+Utilerias.validarCadenaSQL(dtoIn.getFechaIni())+"',103) ");
	        }
...
//	        sql.append( "\n AND fecha_pago is null ");
	        ...
	        //Agregado EMS: 09/11/2015 - Para que solo muestre propuestas con la firma 0 � 1 firma, no 2.... 23/12/15 - Que siempre no, se deben de mostrar aunque ya tenga todas las firmas.
	        ...
	        if(dtoIn.getTipoRegla().equals("S")){
	        	...
	        	if(!dtoIn.getReglaNegocio().equals("")){
	        		...
	        		if(dtoIn.getReglaNegocio().equals("***** TODAS *****")){
		        		...
		        		// NUEVA MODIFICACI�N A REGLAS DE NEGOCIOS, SE VALIDA PARA DESCUENTOS Y REEMBOLSOS.
		        		//** DESCUENTO
	        ...
	        list= jdbcTemplate.query(sql.toString(), new RowMapper<SeleccionAutomaticaGrupoDto>(){
				public SeleccionAutomaticaGrupoDto mapRow(ResultSet rs, int idx) throws SQLException {
					...
					if (rs.getDate("fecha_pago")!=null) {
						cons.setFechaPago(funciones.ponerFechaSola(rs.getDate("fecha_pago")));
					} 
	...
	public List<SeleccionAutomaticaGrupoDto> consultarSeleccionAutomaticaStored(ConsultaPropuestasDto dtoIn){
		...
		try{
			...
			storedPropuestas.declareParameter("V_id_usuario", GlobalSingleton.getInstancia().getUsuarioLoginDto().getIdUsuario(), StoredProcedure.IN);
			...
			resultStored = storedPropuestas.execute();
...
//					cons.setColor(rs.getString("color"));
	...
	public List<MovimientoDto>consultarDetalleStored(PagosPropuestosDto dto){
		...
		try{
			...
			sql.append("\n SELECT distinct m.no_docto, m.no_empresa, m.importe_original,");
			sql.append("\n 							  e.desc_estatus, m.no_factura, m.fec_valor,");
		    sql.append("\n                 m.fec_valor_original, cv.desc_cve_operacion,");
		    sql.append("\n                 m.importe AS importe,");
		    sql.append("\n                 m.id_divisa, m.id_forma_pago, fp.desc_forma_pago,");
			...
			sql.append("\n 																				FROM  valor_divisa vd ");
			sql.append("\n 																				WHERE vd.id_divisa = m.id_divisa ");
			sql.append("\n 																				AND vd.fec_divisa =  convert(datetime,'" + funciones.ponerFechaSola(new Date()) + "',103) )");
			sql.append("\n 																	,1) ");
			sql.append("\n 									END as importe_mn,");
			sql.append("\n 							 (SELECT max(per.nombre_corto)");
			sql.append("\n 									FROM  persona per");
			...
			sql.append("\n 									WHERE cb.id_banco = m.id_banco) as banco_pago, ");
			sql.append("\n								(SELECT max(cg.desc_grupo_cupo)");
			sql.append("\n 									FROM  cat_grupo_cupo cg ,grupo_flujo_cupo gfc");
			sql.append("\n									 WHERE cg.id_grupo_cupo = gfc.id_grupo_cupo");
			sql.append("\n									 and gfc.id_rubro  = m.id_rubro");
			sql.append("\n 									) as desc_grupo_cupo,");
			/*sql.append("\n 								/*(SELECT max(cg.desc_grupo_cupo)");
			sql.append("\n 									FROM  cat_grupo_cupo cg ");
			sql.append("\n 									WHERE cg.id_grupo_cupo = (SELECT max(gfc.id_grupo_cupo)");
			sql.append("\n 																		FROM  grupo_flujo_cupo gfc ");
			sql.append("\n 																		WHERE gfc.id_rubro in (SELECT max(mm.id_rubro)");
			sql.append("\n 																														FROM  movimiento mm ");
			sql.append("\n 																														WHERE mm.folio_ref = m.no_folio_det ");
			sql.append("\n 																													)");
			sql.append("\n 																						)");
			...
			sql.append("\n 								(convert(datetime,'" + funciones.ponerFechaSola(new Date()) + "',103) - m.fec_valor_original) as dias,");
			...
			sql.append("\n 									FROM  ctas_banco cbpc");
			sql.append("\n 									WHERE cbpc.no_persona = cast(m.no_cliente as integer)");
			sql.append("\n 									and cbpc.id_divisa = 'DLS')	as NumCheq_CruceDLS, ");
			...
			sql.append("\n 									FROM  ctas_banco b");
			sql.append("\n 									WHERE b.no_persona = m.no_cliente");
			sql.append("\n 									and b.id_divisa = m.id_divisa ) as NumCheqCte, ");
			sql.append("\n 									m.id_servicio_be, m.id_contable");
			sql.append("\n 									,(select equivale_persona from  persona p where p.no_persona = m.no_cliente and p.id_tipo_persona = 'P') as equivale,");
			sql.append("\n 									coalesce(m.no_cheque, 0) as no_cheque, ");
			sql.append("\n 									coalesce(am.usuario_uno, 0) as usuario_uno, ");
			sql.append("\n 									coalesce(am.usuario_dos, 0) as usuario_dos,");
			sql.append("\n 									m.fecha_contabilizacion, m.fec_operacion,");
			sql.append("\n 									invoice_type,");
			sql.append("\n 									'' as id_clabe,");
			sql.append("\n                    m.po_headers");
		    ...
		    sql.append("\n LEFT JOIN  autorizacionesMov am ON (m.no_folio_det = am.no_folio_det)");
		    sql.append("\n LEFT JOIN  proveedor p ON (cast(m.no_cliente as integer) = p.no_proveedor),");
		    sql.append("\n  cat_tipo_operacion co,  cat_cve_operacion cv,  cat_forma_pago fp,  PERSONA p2");
	    
			sql.append("\n WHERE m.no_empresa in (select no_empresa from  grupo_empresa");
		    sql.append("\n                         where id_grupo_flujo = " + dto.getIdGrupoEmpresa() + ")");
		    sql.append("\n AND m.no_cliente not in (select VALOR from  CONFIGURA_SET where indice = 206)");
		    sql.append("\n AND m.id_tipo_movto = 'E'");
		    sql.append("\n AND (m.origen_mov <> 'INV' or m.origen_mov is NULL)");
		    sql.append("\n AND m.id_tipo_operacion = co.id_tipo_operacion ");
		    sql.append("\n AND co.no_empresa = 1");
		    sql.append("\n AND m.id_cve_operacion = cv.id_cve_operacion");
		    sql.append("\n AND m.id_estatus_mov in ('A','N')");
		    sql.append("\n AND m.id_forma_pago = fp.id_forma_pago");
		    sql.append("\n AND m.id_forma_pago in (1,3,5,6,7,9)");
		    sql.append("\n And m.cve_control = '" + Utilerias.validarCadenaSQL(dto.getCveControl()) + "'");
			sql.append("\n AND m.no_cliente = p2.no_persona");
			sql.append("\n AND p2.ID_TIPO_PERSONA = 'P'	  ");
			sql.append("\n order by no_cheque, beneficiario, fec_valor_original");
...
//               sql+= "\n   AND sa.id_grupo_flujo *= f.id_grupo_flujo ";
	...
	public List<LlenaComboGralDto>llenarComboBancoPagBenef(String sDivisa, int noPersona) {
		...
		try {
			...
			//Consultar si tiene pago cruzado, en caso afirmativo no carga la condici�n de divisas.
	...
	public List<LlenaComboGralDto>llenarComboChequeraPag(int idBanco, int noEmpresa, String sDivisa) {
		...
		try {
			...
			//sql.append("\n 		And id_divisa like '" +Utilerias.validarCadenaSQL(sDivisa)+ "'"); COMENTADO 03/02/2016
	...
	public List<LlenaComboGralDto>llenarComboChequeraPagBenef(int idBanco, int noEmpresa, String sDivisa, int idBenef) {
		...
		try {
			...
			sql.append(" SELECT CB.id_chequera as ID, CB.id_chequera as descrip ");
	...
	public List<MovimientoDto> consultarDetPagos(int noCliente, String fecIni, String fecFin) {
		...
		try {
			...
			sql.append(" 	And fec_valor between '"+ Utilerias.validarCadenaSQL(funciones.cambiarFecha(fecIni, true)) 
							+"' And '"+ Utilerias.validarCadenaSQL(funciones.cambiarFecha(fecFin, true)) +"' \n");
...
/*
	...
	*/
	...
	public int updateFecProp(String cveControls, String fecha) {
		...
		try {
			sql.append(" UPDATE seleccion_automatica_grupo \n");
			sql.append(" SET fecha_propuesta = '"+ Utilerias.validarCadenaSQL(fecha) +"' \n");
...
public List<BloqueoPagoCruzadoDto> consultaPagosCruzadosDivisa(String noPersona) {
	...
	public Map<String, String> insertaSubPropuesta(double montoMaximo, String nvaCveControl, String oldCveControl, int idUsuario, String fecha){
		...
		try{
    		...
    		sql.append("\n ) "); //Se quita la primera autorizaci�n EMS: 27/01/16
    		//sql.append("\n ,usuario_uno ) ");
    		...
    		if(count > 0){
    			...
    			resultado.put("error", "No se gener� la subpropuesta.");
    		}
		...
		public Map<String, String> actualizaMultisociedad(String cveControl, 
			...
			try{
				...
				if(cont <= 0){
					...
					resultado.put("msgUsuario", "Banco/Chequera multisociedad actualizada con �xito.");
				}
		...
		public int consultaRFC(String cveControl) {
			...
			try{
				...
				for(int i=0; i< lstMovto.size(); i++){
					...
					sql.append("\n SELECT count(P.RFC) ");
		    		sql.append("\n FROM PERSONA P ");
		    		sql.append("\n WHERE (NOT Patindex (P.rfc,'^(([A-Z]|[a-z]){4})([0-9]{6})((([A-Z]|[a-z]|[0-9]){3}))')!=0 ");
		    		sql.append("\n AND NOT Patindex (P.rfc,'^(([A-Z]|[a-z]){3})([0-9]{6})((([A-Z]|[a-z]|[0-9]){3}))')!=0) ");
		    		sql.append("\n AND P.id_tipo_persona = 'P' ");
