...
public class PagoPropuestasDaoImpl implements PagoPropuestasDao {
	...
	public Map<String, Object> ejecutarPagador(StoreParamsComunDto dto) throws Exception{
		consultasGenerales = new ConsultasGenerales(jdbcTemplate, transactionTemplate);
...
public List<BloqueoPagoCruzadoDto> obtenerBloqueoProveedor(String noEmpresa, String noProv, String cveControl) {
		...
		try{
            ...
            if(dto.getPsDivision()!=null && !dto.getPsDivision().equals(""))
	           ...
	           // sEnc+="\n            movimiento m, cat_grupo_flujo cgf, grupo_flujo_cupo gfc, ";
				...
				sEnc+="\n        M.id_forma_pago In (1,10)), cat_grupo_flujo cgf, grupo_flujo_cupo gfc, ";
	        }
	       ...
	       sCND+="\n	(m.no_cliente in (select p.no_persona from  persona p  join Conf_Pago_Cruzado cxp on p.equivale_persona = cxp.no_proveedor where p.id_tipo_persona ='P')) or";
	...
	public List<Map<String, String>> consultaPagosExcel(String clave){
		...
		try{
	        ...
	        sql.append( "\n      DATEDIFF(day, convert(datetime,'"+Utilerias.validarCadenaSQL(funciones.ponerFecha(consultarFechaHoy()).substring(0,10)) 
			 +"', 103), m.fec_valor_original) AS dias,");
...
//	        
	...
	public List<MovimientoDto>consultarDetalle(PagosPropuestosDto dto){
		...
		try{
		    ...
		    sql.append( "\n      DATEDIFF(day, convert(datetime,'"+Utilerias.validarCadenaSQL(funciones.ponerFecha(dto.getFecha()).substring(0,10)) 
			 +"', 103), m.fec_valor_original) AS dias,");;
	...
	public String obtieneOrigenMov(String cveControl){
		...
		try {
			sql.append("select distinct case when origen_mov is null then '' else origen_mov end as origen_mov ");
			sql.append("\n from movimiento ");
			sql.append("\n where cve_control in('");
			sql.append(Utilerias.validarCadenaSQL(cveControl));
			sql.append("')");
			System.out.println("oRIGEN MOVIMiENTO "+sql);
	...
	*/
	...
	public List<MovimientoDto> consultarPagosCpaVtaTransferPropManual(String cveControl){
		...
		try{
		    ...
		    sql+= "\n	 and m.id_forma_pago = 3 ";
		    sql+= "\n	 and ltrim(rtrim(m.id_divisa))= ltrim(rtrim(cxp.id_divisa_original))";
	...
	public List<MovimientoDto> consultarPagosBenefDatosAlt(ComunEgresosDto dtoIn){
		...
		try{
		    ...
		    if(gsDBM.equals("POSTGRESQL"))
		    {
		        ...
		        sql+= "                convert       WHERE cbpc.no_persona = m.no_cliente ";
	...
	public int insertarDato(int noEmpresa, String cadenaPagador){
		...
		try{
			...
			System.out.println("datos_pagador_cheq "+sql);
			return jdbcTemplate.update(sql);
		}catch(Exception e){
			bitacora.insertarRegistro(new Date().toString() + " " + Bitacora.getStackTrace(e)
	...
	public int actualizarCveControl(int noEmpresa, String psFolios, String cveControl, Date fecPropuesta){
		...
		try{
		    ...
		    sql+="\n       fec_propuesta = convert(date,'" +funciones.ponerFecha(fecPropuesta)+ "' ,103)";
	...
	public List<Map<String, Object>> reportePagoPropuestasDetalle(PagosPropuestosDto dto){
		...
		try {
		    ...
		    sEnc.append(" when m.id_banco = m.id_banco_benef and m.id_forma_pago=3 then 'Mismo banco' when m.id_banco <> m.id_banco_benef and m.id_forma_pago=3 then 'Interbancario' when (m.id_banco = m.id_banco_benef or m.id_banco <> m.id_banco_benef)  and m.id_forma_pago <> 3 then '' end as mismo_banco, \n");
	        ...
	        listResult = jdbcTemplate.query(sql.toString(), new RowMapper<Map<String, Object>>(){
				public Map<String, Object> mapRow(ResultSet rs, int idx) throws SQLException {
					...
					map.put("desc_forma_pago", rs.getString("desc_forma_pago"));
					map.put("mismo_banco", rs.getString("mismo_banco"));
	    			return map;
				}
			});
	...
	public void actualizarFecPago(String cveControl, String fecHoy) {
		...
		try {
			//System.out.println("  ultimo update ");
			sql.append(" UPDATE seleccion_automatica_grupo SET fecha_pago = '" + fecHoy + "' \n");
			sql.append(" WHERE cve_control = '" + cveControl + "'");
			System.out.println(" ultimo update "+ sql);
	...
	public int inserPagosZexpFact (MovimientoDto folios , int idUsuario){
		...
		try {
			...
			sqlInsert.append("\n m.id_chequera,'' as referencia, m.concepto,'000' as mandante, convert(CHAR(11),m.fec_valor,103) as fecha_exp,'P','',");
			...
			sqlInsert.append("\n (Select convert(char(8),getdate(), 108) From Dual),");
	...
	public DT_Pagos_OBPagos pagosParaWebservice (MovimientoDto folios, String fecHoy, int idUsuario){
		...
		try {
			if(folios.getCveControl()!=null && !folios.getCveControl().equals("")){
				...
				sql.append("\n  Select SOIEMP, NO_DOCTO, SECUENCIA, NO_PEDIDO, NO_FOLIO_DET, ");
				...
				sql.append("\n	,m.id_divisa,m.id_divisa_original,  (select valor from valor_divisa vd where vd.fec_divisa = (select fec_hoy from fechas) and ltrim(rtrim(vd.id_divisa)) = ltrim(rtrim(m.id_divisa_original))) as tipo_camb  ,");
				sql.append("\n  case when M.id_forma_pago in (3,6,8,5,10)  then 3 ");
				sql.append("\n  else case when M.id_forma_pago = 1 then 1  ");
				sql.append("\n  Else M.Id_Forma_Pago End  End as forma_pago,M.Id_Banco,");
				sql.append("\n  '' As Referencia, M.Concepto,'000' As Mandante,");
				sql.append("\n '"+fecHoy+"' As Fechap,0 As Cheque_Trn,");
	...
	public int actualizaMovimientoCompensado(MovimientoDto folios, DT_Pagos_ResponsePagosResponse pagosRespuestaq,String fecHoy){
		...
		int res = 0;
		try {
			sqlActulizaMob = "update movimiento set fec_exportacion = '"+ Utilerias.validarCadenaSQL(fecHoy) +"' ,lote_salida = '1', ";
	...
	public List<MovimientoDto> consultarPagPropApagar(ComunEgresosDto dtoIn){
		...
		List<MovimientoDto>listDatos= new ArrayList<MovimientoDto>();
		try{
			sql+= " SELECT m.id_chequera_benef, m.beneficiario, m.no_empresa, no_cliente as no_proveedor, ";
		    sql+= "\n        id_divisa, id_forma_pago, m.id_banco as id_banco_pag, ";
		    sql+= "\n        m.id_chequera as id_chequera_pago, no_folio_det, no_docto ";
		    ...
		    sql+= "\n    AND (m.origen_mov <> 'INV' or m.origen_mov is null) ";
		    sql+= "\n    AND m.id_estatus_mov in ('N','C','F','L') ";
		 //   sql+= "\n    and (m.PO_HEADERS <> '' or m.PO_HEADERS is not null) ";	
		    sql+= "\n    AND ((m.id_forma_pago = 3 and coalesce(m.id_leyenda, ' ') <> '*') or ";
		    sql+= "\n	(M.No_Cliente in (select no_persona from persona p where id_tipo_persona = 'P' "
		    		+ "						AND equivale_persona in (select no_proveedor from CONF_PAGO_CRUZADO))) or ";

		    sql+= "\n        (m.id_forma_pago in (1, 5, 6, 9, 7, 10))) ";
	...
	*/
	...
	public boolean actualizaCompraTransfer(String cveControl) {
		try {
			...
			bitacora.insertarRegistro(new Date().toString() + " " + Bitacora.getStackTrace(e)
			+ "P:Egresos, C:PagoPropuestasDaoImpl, M:actualizaCompraTransfer");
			e.printStackTrace();
			return false;
		}
	} 
	...
	public boolean ejecutaCVT(String cveControl) {
		try {
			...
			sql.append("UPDATE movimiento \n"
					...
					+ "	AND no_folio_det IN \n"
					+ "		(SELECT distinct no_folio_det \n"
					+ "		FROM movimiento m, cat_cta_banco ccb, ctas_banco cb \n"
					+ "		WHERE m.id_chequera = ccb.id_chequera \n"
					+ "			AND m.id_chequera_benef = cb.id_chequera \n"
					+ "			AND (m.id_servicio_be='CVT' or cb.id_divisa <> ccb.id_divisa) \n"
					+ "			and id_estatus_mov = 'N' \n"
					+ "			and id_tipo_operacion = 3000 \n"
					+ "			AND m.cve_control = '" + cveControl + "') ");
			System.out.println(sql);
			return jdbcTemplate.update(sql.toString()) >= 0;
		} catch(Exception e) {
			e.printStackTrace();
			bitacora.insertarRegistro(new Date().toString() + " " + Bitacora.getStackTrace(e)
			+ "P:Egresos, C:PagoPropuestasDaoImpl, M:ejecutaCVT");
			return false;
		}
	}
}
