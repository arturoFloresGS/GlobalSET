...
public class PagoDePropuestasSetDaoImpl implements PagoDePropuestasSetDao{
	...
	public List<MovimientoDto>consultarDetallePropuestasNoPagadas(PagosPropuestosDto dto){
		...
		try{
	    	...
	    	*/
		    ...
		    sql.append( "\n	  AND m.id_estatus_mov in ('T','C','F','L','I','R') ");
	...
	public List<MovimientoDto> consultarPagProp(PagosPropuestosDto dto){
		...
		try{
	       ...
	       /*if(dto.isBTambienIntercos())
	           ...
	           sCND+="\n AND m.id_estatus_mov in ('T','C','F','I','R') ";
	       */
	      ...
	      /* sCND+="\n     AND ((m.id_forma_pago = 3 and m.id_leyenda <> '*') or ";
	       ...
	       sCND+="\n And ( ";
	       sCND+="\n         (m.id_forma_pago = 3 and m.id_leyenda <> '*' And M.Id_Estatus_Mov In ('T','C','F','I','R') ) or ";
		   sCND+="\n         (M.No_Cliente = (Select Max(P.No_Persona) From  Persona P  ";
		   sCND+="\n                       Join Conf_Pago_Cruzado Cxp On ";
		   sCND+="\n                       P.Equivale_Persona = Cxp.No_Proveedor"; 
		   sCND+="\n                      Where P.Id_Tipo_Persona ='P') And M.Id_Estatus_Mov In ('T','C','F','I','R') ) Or";
		   sCND+="\n                     (M.Id_Forma_Pago In (1, 6, 7, 9) And M.Id_Estatus_Mov In ('T','C','F','I','R') ) Or";
		   sCND+="\n                     (M.Id_Forma_Pago In (5,10) And M.Id_Estatus_Mov In ('N'))";
		   sCND+="\n   )";
		   ...
		   System.out.println("consulta del grid aaaa"+ sql);
...
//	        
	...
	public List<MovimientoDto>consultarDetalle(PagosPropuestosDto dto){
		...
		List<MovimientoDto>listDetalle= new ArrayList<MovimientoDto>();
		try{
			sql.append( "  SELECT distinct m.po_headers,m.no_docto, m.no_empresa, importe_original, ");
		    sql.append( "\n      e.desc_estatus, m.no_factura, m.fec_valor, ");
		    sql.append( "\n      m.fec_valor_original, cv.desc_cve_operacion, ");
		    sql.append( "\n      m.importe AS importe, ");
		    sql.append( "\n      m.id_divisa, m.id_forma_pago, fp.desc_forma_pago, ");
	    	...
	    	*/
		    ...
		    sql.append( "\n	  AND m.id_cve_operacion = cv.id_cve_operacion ");
		    //sql.append( "\n	  AND m.id_estatus_mov in ('T','C','F','L','I','R') ");
		    sql.append( "\n	  and ( (m.id_estatus_mov in ('T','C','F','L','I','R') AND m.id_forma_pago in (1,3,5,6,7,9,10)) or ");
		    sql.append( "\n	 (m.id_estatus_mov in ('N') AND m.id_forma_pago in (5,10)))");
		    sql.append( "\n	  AND m.id_forma_pago = fp.id_forma_pago ");
		    //sql.append( "\n	  AND m.id_forma_pago in (1,3,5,6,7,9,10) ");
		    sql.append("\n   and (m.po_headers is not null or m.po_headers <>'')");
		    /*sql.append( "\n	  AND m.no_folio_det in ");//se comento porque no permitia ver los detalles de pagos parcializados porque no coincidian los folios
	        ...
	        sql.append( "\n	  SELECT distinct m.po_headers,m.no_docto, m.no_empresa, importe_original, ");
	        sql.append( "\n      e.desc_estatus, m.no_factura, m.fec_valor, ");
	        sql.append( "\n      m.fec_valor_original, cv.desc_cve_operacion, ");
	        sql.append( "\n      m.importe AS importe, ");
	        sql.append( "\n      m.id_divisa, m.id_forma_pago, fp.desc_forma_pago, ");
	...
	*/
		  ...
		  //fin parte del codigo
		   ...
		   //     sql+= " LEFT JOIN ctas_banco cbp ON ( cbp.no_persona = convert(int, m.no_cliente) ";
		    ...
		    if (gsDBM.equals("POSTGRESQL"))
		    	...
		    	sql+= " LEFT JOIN persona per ON (m.no_cliente = Cast(Per.No_Persona as Varchar2(15))";
		    ...
		    sql+= "     AND m.id_estatus_mov in ('T','C','F','I','R')";
		    sql+= "     and (m.PO_HEADERS <> '' or m.PO_HEADERS is not null) ";
	...
	public int contarMovsPropMAut(ComunEgresosDto dto){
		...
		try{
		    ...
		    sql+="\n     AND ( m.origen_mov <> 'INV' or m.origen_mov is null )";
		    sql+="\n     AND m.id_estatus_mov in ('T','C','F','L','V','I','R') ";
	...
	public List<String> consultarPagoMassPayment(int noFolioDet, Date fechaHoy){
		...
		try{
		    ...
		    sql+="\n   AND c.pago_mass = 'S'";
		    sql+="\n   AND m.id_tipo_operacion = 3000";
		    sql+="\n   AND m.id_estatus_mov in ('T','C','I','R')";
		    sql+="\n   AND m.id_banco = 2";
	...
	public List<Map<String, Object>> reportePagoPropuestas(PagosPropuestosDto dto){
		...
		try{
	       ...
	       //sCND+="\n     AND ((m.id_forma_pago = 3 and m.id_leyenda <> '*') or ";
    	   //sCND+="\n         (m.id_forma_pago in (1, 5, 6, 7, 9,10))) ";
	       sql+= "     and (m.po_headers is not null or m.po_headers <>'')";
    	   sCND+="\n     AND ((m.id_forma_pago = 3 and m.id_leyenda <> '*') or ";
	       sCND+="\n	(m.no_cliente = (select max(p.no_persona) from  persona p  join Conf_Pago_Cruzado cxp on p.equivale_persona = cxp.no_proveedor where p.id_tipo_persona ='P')) or";
    	   sCND+="\n         (m.id_forma_pago in (1, 5, 6, 7, 9,10))) ";
	...
	public List<Map<String, Object>> reportePagoPropuestasDetalle(PagosPropuestosDto dto){
		...
		try {
	        ...
	        if(dto.getTipoReporte() == 2) {
		        if(dto.isBTambienIntercos())
		            sCND.append(" and m.id_estatus_mov in ('T','C','F','L','I','R') \n");
		        else
		            sCND.append(" and m.id_estatus_mov in ('T','C','F','I','R') \n");
	        }else if(dto.getTipoReporte() == 3) {
	        	if(dto.isBTambienIntercos())
		            sCND.append(" and m.id_estatus_mov in ('T','A','V','L','I','R') \n");
		        else
		            sCND.append(" and m.id_estatus_mov in ('T','A','V','I','R') \n");
	        }
	        
	        sCND.append(" and (m.po_headers is not null or m.po_headers <>'')");
	...
	public List<MovimientoDto> consultarPagPropApagar(ComunEgresosDto dtoIn){
		...
		List<MovimientoDto>listDatos= new ArrayList<MovimientoDto>();
		try{
			sql+= " SELECT m.beneficiario, m.no_empresa, no_cliente as no_proveedor,m.cve_control, m.id_chequera_benef,";
		    sql+= "\n        id_divisa, id_forma_pago, m.id_banco as id_banco_pag, ";
		    sql+= "\n        m.id_chequera as id_chequera_pago, no_folio_det, no_docto, c_lote ";
		   ...
		   // sql+= "\n    AND m.id_estatus_mov in ('T','C','F','I','R') ";
		    ...
		    sql+= "\n and (m.PO_HEADERS <> '' or m.PO_HEADERS is not null)";
    		sql+= "\n AND ((m.id_forma_pago = 3 and coalesce(m.id_leyenda, ' ') <> '*' and m.id_estatus_mov in ('T','C','F','I','R') ) or";
    		sql+= "\n    (M.No_Cliente = (Select Max(P.No_Persona) From  Persona P  ";
    		sql+= "\n Join Conf_Pago_Cruzado Cxp On P.Equivale_Persona = Cxp.No_Proveedor ";
		    sql+= "\n where p.id_tipo_persona ='P' and rtrim(ltrim(m.id_divisa)) = rtrim(ltrim(cxp.id_divisa_pago))";
		    sql+= "\n                )";
		    sql+= "\n  ) or";
		    sql+= "\n ((m.id_forma_pago in (1, 5, 9, 7, 10)) and (m.id_estatus_mov in ('T','C','F','I','R')) ) or ";
		    sql+= "\n (m.id_forma_pago in (5,10) and  m.id_estatus_mov in ('N') ))";
		    ...
		    sql+= "\n	(M.No_Cliente = (Select Max(P.No_Persona) From  Persona P  ";
		    sql+= "\n	Join Conf_Pago_Cruzado Cxp On P.Equivale_Persona = Cxp.No_Proveedor ";
		    sql+= "\n	where p.id_tipo_persona ='P' and trim(m.id_divisa) = trim(cxp.id_divisa_pago))) or ";
...
//		    sql+= "\n    AND m.fec_propuesta = '"+funciones.ponerFecha(dtoIn.getFechaPago())+"'";
	...
	public int actualizaMovtoTC(int idUsuario, String fecModif, Double tipoCambio, String foliosDet,
		...
		try {
			...
			sql.append(foliosDet);
			sql.append("))");
			sql.append(" and id_estatus_mov in('T','C','H')");
	...
	public List<ComunEgresosDto> consultarPropAgrup(List<ComunEgresosDto> dtoIn){
		...
		try {
			for (int i = 0; i < dtoIn.size(); i++) {
				...
				sql.append(dtoIn.get(i).getCveControl());
				sql.append("')");
