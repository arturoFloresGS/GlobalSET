...
import com.webset.set.egresos.dao.PagoDePropuestasSetDao;
...
public class PagoDePropuestasSetBusinessImpl implements PagoDePropuestasSetService {
	private PagoDePropuestasSetDao pagoDePropuestasSetDao;
	...
	public List<LlenaComboGralDto> llenarComboGrupoEmpresa(GrupoEmpresasDto dto) {
		return pagoDePropuestasSetDao.llenarComboGrupoEmpresa(dto);
	}
	...
	public SaldosChequerasDto obtenerSaldosChequeras(SaldosChequerasDto dto){
		...
		try{
			dto.setFecha(pagoDePropuestasSetDao.obtenerFechaHoy());
			listSalCheq=pagoDePropuestasSetDao.obtenerSaldosChequeras(dto);
			...
			if(listSalCheq.size() > 0)
			{
				for(int i=0; i<listSalCheq.size(); i++){
					if(listSalCheq.get(i).getIdDivisa() != null && !listSalCheq.get(i).getIdDivisa().equals("") 
							&& listSalCheq.get(i).getIdDivisa().equals(pagoDePropuestasSetDao.consultarConfiguraSet(771)))
					{
		                ...
		                saldoFinalMN = saldoFinalMN + (listSalCheq.get(i).getSaldoFinal() != 0 ? listSalCheq.get(i).getSaldoFinal() : 0);
							...
							&& listSalCheq.get(i).getIdDivisa().equals(pagoDePropuestasSetDao.consultarConfiguraSet(772)))
	                {
	...
	public List<MovimientoDto>consultarDetallePropuestasNoPagadas(SeleccionAutomaticaGrupoDto dtoIn){
		...
		try{ 
			if(dtoIn.getCveControl().trim().substring(0,1).equals("M")){
				...
				consDetalleDto.setFecha(pagoDePropuestasSetDao.consultarFechaHoy());//Checar
				consDetalleDto.setPsDivision("");
				listDetalle=pagoDePropuestasSetDao.consultarDetallePropuestasNoPagadas(consDetalleDto);
			}
	...
	public List<MovimientoDto>consultarDetalle(SeleccionAutomaticaGrupoDto dtoIn){
		...
		try{ 
			if(dtoIn.getCveControl().trim().substring(0,1).equals("M")){
				...
				consDetalleDto.setFecha(pagoDePropuestasSetDao.consultarFechaHoy());//Checar
				consDetalleDto.setPsDivision("");
				listDetalle=pagoDePropuestasSetDao.consultarDetalle(consDetalleDto);
			}
			...
			if(listDetalle.size() > 0) {
				for(int i=0; i<listDetalle.size(); i++) {
					...
					listBloqueados = pagoDePropuestasSetDao.obtenerBloqueoProveedor(
					...
					if(listBloqueados.size()>0){
						...
						listBloqueados = pagoDePropuestasSetDao.obtenerBloqueoSAP(
								""+dtoIn.getCveControl(),
					...
					count = pagoDePropuestasSetDao.existeChequeraBenef(listDetalle.get(i));
	...
	public List<MovimientoDto> consultarPropuestas(PagosPropuestosDto dtoIn) {
		...
		try {
			dtoIn.setFechaHoy(pagoDePropuestasSetDao.obtenerFechaHoy());
			//System.out.print(pagoDePropuestasSetDao.obtenerFechaHoy());
	        dtoIn.setAutorizaPropuesta(pagoDePropuestasSetDao.consultarConfiguraSet(240));
	        ...
	        listCons = pagoDePropuestasSetDao.consultarPagProp(dtoIn);
	...
	public void acutalizarConfContablePag(String cveControl) {
		...
		try {
			listDatPag = pagoDePropuestasSetDao.buscaDatosConfigContable(cveControl);
    		
    		if(listDatPag.size() > 0) {
    			for(int i=0; i<listDatPag.size(); i++) {
    				listFolioDetPag = pagoDePropuestasSetDao.buscaFolioDatosPagados(cveControl, listDatPag, i);
    				
    				if(listFolioDetPag.size() > 0) {
    					pagoDePropuestasSetDao.actFolioPagConfigCont(listFolioDetPag.get(0).getNoFolioDet(), 
			...
			if(listGrid.size()>0)
			{
				for(int i=0; i<listGrid.size();i++)
				{
							...
							&& listGrid.get(i).getCveControl().trim().substring(0,1).equals("M"))
					{
						...
						listPagCpaVtaTran=pagoDePropuestasSetDao.consultarPagosCpaVtaTransferPropManual(listGrid.get(i).getCveControl());
						...
						dtoParam.setFechaHoy(pagoDePropuestasSetDao.obtenerFechaHoy());
						...
						listPagCruzadosAut=pagoDePropuestasSetDao.consultarPagosCruzadosAut(dtoParam);
		    			...
		    			&& listGrupos.get(index-1).getBeneficiario().equals(psBeneficiario))
		    	{
		    		if(listGrupos.get(index-1).getIdFormaPago()==1)
		    		{
		    			if(psCveControl.trim().substring(1,2).equals("G"))
		    			{
		    				...
		    				if(pagoDePropuestasSetDao.consultarConfiguraSet(220).equals("NO"))
		    				{
			...
			if (listGrid.size()>0) {
				for (int i = 0; i < listGrid.size(); i++) {
					if(listGrid.get(i).getCveControl()!=null && !listGrid.get(i).getCveControl().equals("") 
						...
						listPagProApagar=pagoDePropuestasSetDao.consultarPagPropApagar(dtoParam);
					
					}else if(listGrid.get(i).getCveControl()!=null && !listGrid.get(i).getCveControl().equals("") 
							&& listGrid.get(i).getCveControl().trim().substring(0,1).equals("M")){
						String origenMovimiento = pagoDePropuestasSetDao.obtieneOrigenMov(listGrid.get(i).getCveControl());
						if (origenMovimiento.equals("CAJ") || origenMovimiento.equals("CVD")) {
							//no entra para origen sap
							rstDatos = pagoDePropuestasSetDao.consultarPagosCpaVtaTransferPropManual(listGrid.get(i).getCveControl());
							if (rstDatos.size() > 0) {
								psFolios = "";
							...
							listMovto=pagoDePropuestasSetDao.consultarPagosCpaVtaTransferPropManual(listGrid.get(i).getCveControl());
							if (listMovto.size()>0) {
								psFolios = "";
								psDocumentosCompensar  ="";
								psDoctosCruzarAut = "";
						...
						if (!psDoctosCruzarAut.equals("")) {
							if(!psFoliosWL.equals(""))
								lAfec=pagoDePropuestasSetDao.actualizarEstatusComVtaTransfer(psFoliosWL, true);
							if(!psFolios.equals(""))
								lAfec=pagoDePropuestasSetDao.actualizarEstatusComVtaTransfer(psFolios,false);
							if (!psFoliosCruzado.equals("")) 
								lAfec = pagoDePropuestasSetDao.actualizaMovtoTC(globalSingleton.getUsuarioLoginDto().getIdUsuario(),
									...
									for(int ji=0; ji<listPagCruzadosAut.size();ji++){
												...
												&& (listPagCruzadosAut.get(ji).getIdDivisa()!=null && listPagCruzadosAut.get(ji).getIdDivisa().equals("DLS")))
										{
											...
											if (listPagCruzadosAut.get(ji).getIdFormaPago() == 3 || listPagCruzadosAut.get(ji).getIdFormaPago() == 5) {
												lAfec=pagoDePropuestasSetDao.actualizarBancoCheqBenef(listPagCruzadosAut.get(ji).getNoFolioDet(),
											...
											if(!psFolios.equals("")){
													...
													mapCuadrante=pagoDePropuestasSetDao.cambiarCuadrantes(cambiaCuadranteDto);
												...
												&& (listPagCruzadosAut.get(ji).getIdDivisa()!=null && listPagCruzadosAut.get(ji).getIdDivisa().equals("DLS")))
										{
	                                         ...
	                                         if(listPagCruzadosAut.get(ji).getIdFormaPago()==3
	                                            ...
	                                            listBanCheqBenef = pagoDePropuestasSetDao.consultarBancoCheqBenef(paramBanCheqBenef);
	                                            ...
	                                            lAfec = pagoDePropuestasSetDao.actualizarBancoCheqBenef(paramBanCheqBenef);
						...
						listPagosBenefDatosAlt= pagoDePropuestasSetDao.consultarPagosBenefDatosAlt(dtoParam);
						...
						listPagProApagar=pagoDePropuestasSetDao.consultarPagPropAut1(dtoParamGral);//llama a consultarPagPropAut1
						
						dtoParam= new ComunEgresosDto();
						dtoParam.setCveControl(listGrid.get(i).getCveControl());
						ctaDatos=pagoDePropuestasSetDao.contarMovsPropMAut(dtoParam);
					...
					if(listPagProApagar.size()>0){
						for(int cont=0; cont<listPagProApagar.size();cont++){
							...
							if(listGrupos.size()==0 
								...
								idxGrupo++;
									if(pagoDePropuestasSetDao.consultarConfiguraSet(261).equals("SI")){
										List<String>listMassPay= new ArrayList<String>();
										
										listMassPay=pagoDePropuestasSetDao.consultarPagoMassPayment(listPagProApagar.get(cont).getNoFolioDet(),
												pagoDePropuestasSetDao.obtenerFechaHoy());
						...
						//						piNumFolioMassActual = 1;
							//					piNumArregloMassActual ++;
				                           ...
				                           // if(ubicacion.equals("CCM") && listPagProApagar.get(cont).getIdBancoPago()==2
				                            //		&& listGrid.get(i).getFechaPago()<=pagoDePropuestasSetDao.obtenerFechaHoy()){
						...
						if(!foliosMass.equals("")){
								pagoDePropuestasSetDao.actualizarEstatusMass(foliosMass);
						}
					}
				}//for
	...
	public Map<String,Integer> llamarPagadorFactoraje(int noUsuario, String sFolios, int iFoliosRech, String psMensaje, int noEmpresa){
		...
		try{
		    plFolio = pagoDePropuestasSetDao.seleccionarFolioReal("no_folio_param");
		    plFolio2 = pagoDePropuestasSetDao.seleccionarFolioReal("no_folio_param");
		    dtoFactor.setNoUsuario(noUsuario);
		    dtoFactor.setSFolios(sFolios);
		    dtoFactor.setPlFolio(plFolio);
		    dtoFactor.setPlFolio2(plFolio2);
		    dtoFactor.setFechaHoy(pagoDePropuestasSetDao.obtenerFechaHoy());
		    
		    afecFactoraje=pagoDePropuestasSetDao.insertarParametroFactoraje(dtoFactor);
		    if(afecFactoraje>0)
		    {
		    	resp = pagoDePropuestasSetDao.ejecutarGenerador(noEmpresa, plFolio, pdFolioMov, pdFolioDet, 1, "inicia generador");
		    	...
		    	//pagoDePropuestasSetDao.actualizarSolicitudFactoraje(noFolioDet);
	...
	public JRDataSource reportePagoPropuestas(PagosPropuestosDto dtoIn){
		...
		try {
	        ...
	        if(dtoIn.getTipoReporte() == 1)
	        	listReport = pagoDePropuestasSetDao.reportePagoPropuestas(dtoIn); 
	        else if(dtoIn.getTipoReporte() == 2 || dtoIn.getTipoReporte() == 3)
	        	listReport = pagoDePropuestasSetDao.reportePagoPropuestasDetalle(dtoIn);
	...
	public PagoDePropuestasSetDao getpagoDePropuestasSetDao() {
		return pagoDePropuestasSetDao;
	}

	public void setpagoDePropuestasSetDao(PagoDePropuestasSetDao pagoDePropuestasSetDao) {
		this.pagoDePropuestasSetDao = pagoDePropuestasSetDao;
	}
	...
	public Map<String, Object> insertarFoliosZexpFact(String datos, String fecHoy, int idUsuario) {
		...
		try {
			List<ComunEgresosDto> listGrid = 
					convierteJsonToListComunEgresosDto(datos);
			...
			if(listGrid.size()>0){
				...
				if (listGrupos != null && listGrupos.size() > 0) {
					...
					for (int i = 0; i < listGrupos.size(); i++) {
						...
						respuestaExportar.put("Clave Control", listGrupos.get(i).getCveControl());										
						
						
						Map<String, Object> respuestaPagador = ejecutarPagador(
								listGrupos.get(i));
						if (!(Boolean)respuestaPagador.get("estatus")) {
							respuestaExportar.put("Mensaje",respuestaPagador.get("mensaje").toString());
							respuestaExportar.put("estatus","No se pago");
							respuestaExportar.put("origen","SET");
							//mapRetorno.put("respuesta", "(SET) Los datos no se pagaron exitosamente.");
						}else{
							respuestaExportar.put("Mensaje",respuestaPagador.get("mensaje").toString());
							respuestaExportar.put("estatus","Se pago");
							respuestaExportar.put("origen","SET");
							//mapRetorno.put("respuesta", "(SET) Los datos se pagaron exitosamente.");
						}
	...
	private Map<String, Object> ejecutarPagador(MovimientoDto movimientoDto) {
		...
		try {
            ...
            if((iBanco==0 || sChequera.equals(""))
            	...
            	if(pagoDePropuestasSetDao.consultarConfiguraSet(1).equals("CCM")) {
            		sCadenaPagador="";
                    ...
                    pagoDePropuestasSetDao.insertarDato(movimientoDto.getNoEmpresa(), sCadenaPagador);                   
            	}else{
            		if(pagoDePropuestasSetDao.consultarConfiguraSet(286).equals("SI")
            			...
            			movto3=pagoDePropuestasSetDao.actualizarMovto3000(dtoMov3);//*in
            			...
            			lResp=pagoDePropuestasSetDao.ejecutarPagador(dtoPagador);
            		}
            	}
            	//{result=0, mensaje=null}
            	if((!pagoDePropuestasSetDao.consultarConfiguraSet(286).equals("SI"))
            			|| (pagoDePropuestasSetDao.consultarConfiguraSet(286).equals("SI") && movimientoDto.getIdFormaPago()!=1))
            	{
            		if((!lResp.isEmpty() && Integer.parseInt(lResp.get("result").toString())==0) 
	            			...
	            			|| movto3==0){
            			pagoDePropuestasSetDao.actualizarCveControl(movimientoDto.getNoEmpresa(), movimientoDto.getPsFolios(), //sFolios
            	...
            	/* if(psMensaje.trim().equals("")) {
            		pagoDePropuestasSetDao.actualizarFecPago(movimientoDto.getCveControl(), fecHoy);
            		acutalizarConfContablePag(movimientoDto.getCveControl());
            	 }else{		            	
					pagoDePropuestasSetDao.actualizarFecPago(movimientoDto.getCveControl(), fecHoy);
            		acutalizarConfContablePag(movimientoDto.getCveControl());
	            }
	...
	public HSSFWorkbook consultaPagosExcel(String clave) {
		...
		try {
			...
			hb=Utilerias.generarExcel(new String[]{
					...
					pagoDePropuestasSetDao.consultaPagosExcel(clave), new String[]{
					"cve_control",
					"no_docto",
					"importe",
					"id_divisa",
