...
//		NS.criterios.banco = NS.id_banco;
	...
	NS.cancelaSolicitud = function(){
		...
		for (var i = 0; i < registroSeleccionado.length; i++) {
			...
			else if (registroSeleccionado[i].get('usr1') != '' || registroSeleccionado[i].get('usr2') != '')
		...
		listeners: {
			load: function(s, records){
				...
				else
				{
          			...
          			Ext.getCmp(PF+'txtMontoTotal').setValue(NS.formatNumber(NS.montoTotal));
...
//                               					NS.storeEnvioTransGrid.baseParams.chkH2HSantander = false;
	...
	//			              			   	'ACM 23(08/07 - Valida que si el Traspaso tiene indicador 'N' sólo se page en la fecha contable indicada (fecha_propuesta)
	//									    'Inicio -------
						          			for(var j = 0; j < regSelec.length; j=j+1)
						          			{
						          				if(regSelec[j].get('habilitado') === 'N' && cambiarFecha(''+regSelec[j].get('fechaPropuesta')) != NS.fechaHoy)
						          				{
									                ...
									                //NS.gridConsulta.deselectRow(regSelec[j]);
	//								                falta programar esta linea para deseleccionar del grid
	//								                msfDatos.TextMatrix(liLinea, LI_C_SELECCION) = ""
									            }
						          			}
						          			if(NS.lbDesmarcados == true)
						          			{
	//									        Ext.Msg.confirm('SET','Se desmarcaron Traspasos que no se pueden ejecutar el día de hoy, '+
	...
	//									    'Fin ----------
										    ...
										    {
									            if(regSelec[i].get('idTipoOperacion') == 3809 && cambiarFecha(''+regSelect[i].get('fecOperacion')) > NS.fechaHoy)
	...
	//								                For i = 1 To msfDatos.Rows - 1
								        	...
								        	for(var i=0;i<regSelec.length;i++)
							           		...
							           		TraspasosAction.ejecutarSolicitudes(jsonString, function(result, e) {
			              			   ...
			              			   /*
			              			   el configura set 232 esta vacio por lo tanto no se cumliran las siguientes condiciones
			              			   y pasa directo al metodo aceptar. por el momento se quedan pendientes estas validaciones
			              			   
									    lbEnvioBitalH2H = False
									    If LI_BANCO = BITAL And gobjVarGlobal.valor_Configura_set(232) = "SI" Then
									        lbEnvioBitalH2H = True
									    End If
									    fraTipoEnvioBital.Visible = False
									    fraTipoEnvioBanamex.Visible = True
									    If msfDatos.Rows > 1 Then
									        If habilita_seleccion_banamex = True Or lbEnvioBitalH2H = True Then
									            If LI_BANCO = BANAMEX Or LI_BANCO = CITIBANK_DLS _
									                Or lbEnvioBitalH2H = True Then
									                If lbEnvioBitalH2H = True Then
									                    fraTipoEnvioBital.Visible = True
									                    fraTipoEnvioBanamex.Visible = False
									                ElseIf LI_BANCO = BANAMEX Then
									                    optBanamexNormal.Value = True
									                End If
									                fraBanamex.Visible = True
									                cmdAceptar.Enabled = False
									                cmdPagoParcial.Enabled = False
	...
	NS.autorizar = function() {
		ConsultaPropuestasAction.validaFacultad(140, function(result, e) {
			if(result == 0) {
