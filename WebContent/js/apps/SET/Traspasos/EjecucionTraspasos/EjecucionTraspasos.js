...
//	NS.storeBeneficiarios.load();
	...
	NS.optEnvioBNMX = new Ext.form.RadioGroup({
		...
		items: [
	          ...
	          {boxLabel: 'BancaNet', name: 'optBNM', inputValue: 0, checked: false},  
	          ...
	          {boxLabel: 'Mass Payment Citibank', name: 'optBNM', hidden:false, inputValue: 3, disabled: false	},
	          {boxLabel: 'Flat File Citibank (Solo DLS)', name: 'optBNM', inputValue: 4, disabled: true	},  
	          {boxLabel: 'Pay Link Citibank (Solo MN)', name: 'optBNM', inputValue: 5, checked: true, disabled: false	},
...
//								                For i = 1 To msfDatos.Rows - 1
	    	...
	    	for(var i=0;i<regSelec.length;i++)
	       	{
	    		if(regSelec[i].get('idBancoBenef') != Ext.getCmp(PF+'cmbValor').getValue() && 
	    			...
	    			Ext.Msg.alert('SET','No se puede continuar.' 
	    					+ 'El registro ' + (i + 1) + ' no tiene cuenta clabe.');
	    			winTipoEnvio.hide();
	    			return;
	    		}
	       		var parametro = {};
	 ...
	 //  		console.log("bandera "+NS.bandera.getValue());
	   	//	console.log("tipo envio "+optValorBNMX);
	   		TraspasosAction.ejecutarSolicitudes(jsonString,optValorBNMX,NS.bandera.getValue(), function(result, e) {
				BFwrk.Util.msgWait('Ejecutando...', false);
				NS.contEnvioBancos.setVisible(false);
