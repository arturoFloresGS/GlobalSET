...
Ext.onReady(function(){
	...
	var NS = Ext.namespace('apps.SET.Impresion.Impresion.ReimpresionCheques');
	...
	NS.storeConfiguracion = new Ext.data.DirectStore({
		...
		root: '',
		paramOrder:[],
		directFn: ReimpresionAction.llenarComboConfiguracion, 
		idProperty: 'id', 
		fields: [
			 {name: 'id'},
			 {name: 'descripcion'}
	...
	NS.storeMotivos = new Ext.data.DirectStore({
		...
		root: '',
		paramOrder:[],
		directFn: ReimpresionAction.llenarComboMotivos, 
		idProperty: 'id', 
		fields: [
			 {name: 'id'},
			 {name: 'descripcion'}
	...
	NS.cmbMotivos = new Ext.form.ComboBox({
		...
		,listeners:{
			select:{
				fn: 
				function(combo, valor) {
					//BFwrk.Util.updateComboToTextField(PF+'txtMotivos',NS.cmbMotivos.getId());
					//NS.accionarCmbMotivos(combo.getValue());
				}
			}
		}
	});
	...
	NS.storeEmpresa = new Ext.data.DirectStore({
		...
		baseParams:{},
		paramOrder:[],
		directFn: ReimpresionAction.llenarComboEmpresa,
	...
	NS.storeBancoCharola = new Ext.data.DirectStore({
		...
		directFn: ReimpresionAction.llenarComboBancoImpLaser,
		idProperty: 'noBanco',
		fields: [
			{name: 'noBanco'},
			{name: 'descBanco'}
	...
	NS.storeLeyenda = new Ext.data.DirectStore({
		...
		baseParams:{},
		paramOrder:['caja'],
		directFn: ReimpresionAction.llenarComboLeyenda, 
		idProperty: 'id', 
		fields: [
			 {name: 'id'},
			 {name: 'descripcion'}
	...
	NS.storeBanco = new Ext.data.DirectStore({
		...
		directFn: ReimpresionAction.llenarComboBancos, 
		idProperty: 'id', 
		fields: [
			 {name: 'id'},
			 {name: 'descripcion'}
	...
	NS.storeBancoCC = new Ext.data.DirectStore({
		...
		root: '',
		paramOrder:['idBanco'],
		directFn: ReimpresionAction.llenarComboBancosCC, 
		idProperty: 'id', 
		fields: [
			 {name: 'id'},
			 {name: 'descripcion'}
	...
	NS.storeChequera = new Ext.data.DirectStore({
		...
		directFn: ReimpresionAction.llenarComboChequera, 
	...
	NS.storeProveedor = new Ext.data.DirectStore({
		...
		baseParams: {texto : ''},
		paramOrder: ['texto'],
		directFn: ReimpresionAction.consultarProveedores, 
		idProperty: 'id', 
		fields: [
			 {name: 'id'},
			 {name: 'descripcion'}
	...
	NS.storeUnicoBeneficiario = new Ext.data.DirectStore({
		...
		root: '',
		directFn: ReimpresionAction.llenarComboBeneficiario, 
		idProperty: 'id', 
		fields: [
			 {name: 'id'},
			 {name: 'descripcion'},
			...
			select:{
				fn:function(combo, value) {
					...
					if(combo.getValue() == 0)
					{
						...
						Ext.getCmp(PF+'txtNoChequeIni').hide();
						Ext.getCmp(PF+'txtNoChequeFin').hide();
					...
					/*else if(combo.getValue() == 9){
						...
						Ext.getCmp(PF+'txtLote').hide();
						Ext.getCmp(PF+'txtDocto').hide();
						Ext.getCmp(PF+'txtNoChequeIni').show();
						Ext.getCmp(PF+'txtNoChequeFin').show();
						Ext.getCmp(PF+'txtSolicitudIni').hide();
						Ext.getCmp(PF+'txtSolicitudFin').hide();
						Ext.getCmp(PF+'txtTipoEgreso').hide();
						...
						var recordsDatGrid=NS.storeDatos.data.items;
						for(i=0;i<recordsDatGrid.length;i++){
							if(recordsDatGrid[i].get('criterio')=='RANGO NO. CHEQUE')
								indice=i;
						}
	...
	NS.storeFirmas = new Ext.data.DirectStore({
		...
		directFn: ReimpresionAction.consultarFirmasChequera, 
	...
	NS.colImpresionCheques = new Ext.grid.ColumnModel ([
		...
		{header :'No. Cheque a Imprimir', width :100, sortable :true, dataIndex :'noChequeTmp'}
	]); 
		...
		if(recordsDatosGrid.length>0){
			...
			for(var i=0;i<recordsDatosGrid.length;i=i+1){
 				...
 				*/
  				...
  				}else if(recordsDatosGrid[i].get('criterio')==='RANGO NO. CHEQUE'){
			...
			if(NS.cEmpresa && NS.cBanco && NS.cChequera){
				...
				/*ReimpresionAction.obtieneUltimoCheqImp(NS.chequera, function(res, e){
				...
				*/
				...
				ReimpresionAction.getUltimoImpCtrlCheque(NS.psNoEmpresa, NS.banco, NS.chequera, function(res, e){
					if(res == '-1'){
                ...
                items: [
                    {
                        ...
                        items: [
                            ...
                            {
                                xtype: 'numberfield',
                                id: PF+'txtNoChequeIni',
                                name: PF+'txtNoChequeIni',
                                x: 100,
                                y: 80,
                                width: 100,
                                hidden: true,
                            ...
                            {
                                xtype: 'numberfield',
                                id: PF+'txtNoChequeFin',
                                name: PF+'txtNoChequeFin',
                                x: 220,
                                y: 80,
                                width: 100,
                                hidden: true,
                                listeners:{
		                        	change:{
		                        		fn:function(caja, valor){
											...
											var datosClase = NS.gridDatos.getStore().recordType;
											var recordsDatGrid=NS.storeDatos.data.items;		
											var valorUno=Ext.getCmp(PF+'txtNoChequeIni').getValue();
											if(valorUno!='')
												valorUno+=' a '
											for(i=0;i<recordsDatGrid.length;i++)
											{
											...
											if(parseInt(valorUno) > parseInt(valor))
											{
												Ext.Msg.alert('SET','El de cheque dos debe ser mayor o igual al numero de cheque uno');
												Ext.getCmp(PF+'txtNoChequeIni').setValue('');
												Ext.getCmp(PF+'txtNoChequeFin').setValue('');
												return;
											}
									    	var datos = new datosClase({
									    			criterio: 'RANGO NO. CHEQUE',
                        		...
                        		fn:function(e){
                        			...
                        			if(recordsGrid.length != null || recordsGrid.length > 0){
                        				...
                        				if(registros.length != 0){
                        					...
                        					/*if(recordsImpresora.length == null || recordsImpresora.length <= 0){
		                        				Ext.Msg.alert('SET','No hay impresoras asignadas a esta caja, no se pueden imprimir cheques');
		                        				return;
	                        				}*/
	                        				
	                        				/*if(NS.numImpresora == ''){
	                        					Ext.Msg.alert('SET','Es necesario seleccionar una impresora');
	                        					return;
	                        				} */	      
	                        				...
	                        				var observacion = NS.cmbMotivos.getRawValue()
	                        				
	                        				if(observacion == ''){
	                        					Ext.Msg.alert('SET','Por favor elija un motivo de reimpresión.');
	                        					//mascara.hide();
	                        					return;
	                        				}
								...
								if(result === true){
									...
									if(NS.psbImpreContinua === 'N'){	//si es impresion laser muestra ventana de charolas
										...
										if(regCheq.length > 0) {
											...
											/*ReimpresionAction.obtieneUltimoCheqImp(chequera, function(res, e){
												Ext.getCmp(PF + 'txtCheque').setValue(res + '');
												NS.ventanaNumCheque.show();
											});*/
	...
	NS.ventanaNumCheque = new Ext.Window({
		...
		NS.storeFolios = new Ext.data.DirectStore({
			...
			directFn: ReimpresionAction.consultarFoliosPapel,
	...
	NS.imprimir = function(){
		...
		for(var i = 0; i < registros.length; i ++){
			...
			registro.observacion = NS.cmbMotivos.getRawValue();
		...
		criterio.noChequeIni =  NS.noChequeIni;
		criterio.noChequeFin = NS.noChequeFin;
		...
		ReimpresionAction.ejecutarImprimirCheques(jsonStringR, jsonStringC, function(result, e){
			...
			if(result.error!==null  &&  result.error!=='' && result.error!= undefined){
				...
				if(result.datos!==null  &&  result.datos!=='' && result.datos !== undefined){
					...
					if(NS.psbImpreContinua == 'S'){
						...
						var ventana = window.open('', 'Print Panel', 'location=middle,width=100, height=100');
						...
						*/
                        ...
                        //win.document.write('Prueba de impresion para laserrr');
                        win.document.write('</body>');
                        win.document.write('</html>');
                        win.document.close();
                        win.print();
	...
	NS.imprimeCheque = function() {
		...
		ReimpresionAction.escribirExcel(datosGrid, function(resp, e){
