Ext.onReady(function(){
	...
	Ext.QuickTips.init(); 
	var NS = Ext.namespace('apps.SET.PrestamosInterempresas.SolicitudDeTraspasosDeCredito');
	...
	NS.limpiar = function()
	{
		NS.SolicitudDeTraspasoDeCredito.getForm().reset();
		Ext.getCmp(PF + 'txtConcepto').setValue("TRASPASO CREDITO");
	...
	//store para el combo de la empresa concentradora
	NS.storeEmpConcen = new Ext.data.DirectStore({
		paramsAsHash: false,
		...
		listeners: {
			load: function(s, records){
				var myMask = new Ext.LoadMask(Ext.getBody(), {store: NS.storeEmpConcen, msg:"Cargando..."});
	...
	NS.cmbEmpresaD = new Ext.form.ComboBox({
		...
		autocomplete: true,
		emptyText: 'Seleccione concentradora',
		triggerAction: 'all',
		value: '',
		listeners:{
			select:{
	...
	NS.storeEmpresaDestino = new Ext.data.DirectStore({
		...
		root: '',
		paramOrder:['iIdEmpresa'],
		directFn: PrestamosInterempresasAction.llenarCmbEmpresaDestino, 
		idProperty: 'id', 
		fields: [
			 {name: 'id'},
			 {name: 'descripcion'}
	...
	NS.crearSolicitud = function(iEmpOrigen, iEmpDestino, iBanOrigen, iBanDestino, 
								 sCheqOrigen, sCheqDestino, uMonto, sConcepto)
	{
		PrestamosInterempresasAction.realizarSolTraspCred(parseInt(iEmpOrigen), parseInt(iEmpDestino), parseInt(iBanOrigen), 
	...
	NS.SolicitudDeTraspasoDeCredito = new Ext.form.FormPanel({
    title: 'Traspaso de credito',
    width: 822,
    height: 468,
    padding: 10,
    layout: 'absolute',
    ...
    autoScroll: true,
    frame: true,
    id: PF + 'SolicitudDeTraspasoDeCredito',
        items : [
            {
                xtype: 'fieldset',
                title: '',
                        ...
                        listeners:
                        {
                        	click:
                        	{
                        		fn: function(e)
                        		{
                        			...
                        			NS.crearSolicitud(NS.iIdEmpresaD, NS.iIdEmpresaA, NS.iIdBancoD,
	...
	NS.SolicitudDeTraspasoDeCredito.setSize(Ext.get(NS.tabContId).getWidth(), Ext.get(NS.tabContId).getHeight());
	NS.limpiar();
});