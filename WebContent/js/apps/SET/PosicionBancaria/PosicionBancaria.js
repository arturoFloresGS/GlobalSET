...
Ext.onReady(function(){
	...
	NS.storeConsultarCriterioSeleccion = new Ext.data.DirectStore( {
		...
		directFn: PosicionBancariaAction.consultarCriterioSeleccion,//revM1
		//idProperty: 'idPlan',
		fields: [
			{name: 'idBanco' },
			{name: 'descBanco'},
	...
	NS.storePlantillas = new Ext.data.DirectStore({
		...
		root: '',
		paramOrder: ['pantalla'],
		directFn: PosicionBancariaAction.llenarComboPlantillas, //rev2
		idProperty: 'id', 
		fields: [
				 {name: 'id'},
				 {name: 'descripcion'}
							...
							fn:function(e){
								...
								for(var i=0; i<itemsPlantillas.length;i++){
									if(itemsPlantillas[i].get('descripcion') == nomPlantilla){
										//Ext.Msg.alert('SET','Se realizará una actualización de la plantilla.');										
									}
								}
	...
	NS.storeMovtoTes = new Ext.data.DirectStore( {
		...
		directFn: PosicionBancariaAction.consultarMovtoTes,//rev3
		//idProperty: 'idPlan',
		fields: [
			{name: 'idBanco'},
			{name: 'descBanco'},
	...
	NS.storeDiarioTes = new Ext.data.DirectStore( {
		...
		root: '',
		paramOrder: ['jsonBancos','jsonChequeras','jsonEmpresas','fechaIni','fechaFin','seleccion','divisa'],
		directFn: PosicionBancariaAction.consultarDiarioTes,//rev4
		//idProperty: 'idPlan',
		fields: [			
			{name: 'descBanco'},
			{name: 'idChequera'},
	...
	NS.storeValorXEmp = new Ext.data.DirectStore({
		...
		directFn: PosicionBancariaAction.posicionXEmp,//rev5
		fields:[
		        'nomReporte',
		        'tipoPosicion',
		        'columnas',
		...
		listeners: {
			load: function(s, records) {
				...
				NS.storeValorXEmp2 = new Ext.data.DirectStore({
					...
					//root: "row",
					paramOrder: ['jsonBancos', 'jsonChequeras','jsonEmpresas','fechaIni','fechaFin','seleccion','divisa','tipoReporte'],
					directFn: PosicionBancariaAction.posicionXEmp2,//rev6
					fields: NS.fields,
					listeners: {
						load: function(s, records) {
							//var myMask = new Ext.LoadMask(Ext.getBody(), {store: NS.storeLlenaGridCash, msg: "Cargando información..."});
        	...
        	dblclick: {
        		fn:function(e) {
        			...
        			if(reg.length > 0){
        				...
        				var rubro = reg[0].get('descripcion').split('-')[0];
	...
	NS.chkSinRubro = new Ext.form.Checkbox({
 		id: PF + 'chkSinRubro',
        name:PF + 'chkSinRubro',
 		boxLabel: 'Sin Rubro',
        x: 680,
	...
	NS.buscarDetallePosicion = function(){
		...
		switch(NS.optBusqueda){
	 	case 1:
	 		...
	 		if(rubros.length <= 0 && !NS.chkSinRubro.getValue()){
	 		...
	 		NS.storeDetallePosicion.baseParams.bSinRubros = NS.chkSinRubro.getValue();
