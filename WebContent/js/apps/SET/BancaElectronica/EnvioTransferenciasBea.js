...
Ext.onReady(function(){
	var NS = Ext.namespace('apps.SET.BancaElectronica.EnvioTransferenciasBea');
	...
	NS.ejecutar = function() {
		...
		for(var i=0; i<registrosSelec.length; i++) {
			var registroGrid = {};
			registroGrid.poHeaders = registrosSelec[i].get('poHeaders');
			registroGrid.noDocto = registrosSelec[i].get('noDocto');
			registroGrid.nomEmpresa = registrosSelec[i].get('nomEmpresa');
			registroGrid.fecOperacion = NS.cambiarFechaGrid(""+registrosSelec[i].get('fecOperacion'));
			...
			registroGrid.fecValor = registrosSelec[i].get('fecValor');
			...
			registroGrid.direccionBenef = registrosSelec[i].get('direccionBenef');
		...
		criterios.chkH2HSantander = NS.chkH2HSantander;
		...
		EnvioTransferenciasAAction.ejecutarEnvioTrans(jsonString, jsonString2, function(result, e) {
			if(result != null && result != undefined && result != '') {
				...
				//Ext.Msg.alert('SET', '' + result.msgUsuario);
				Ext.Msg.alert('Información SET', ''+result.msgUsuario, function(btn){
				    if (btn == 'ok'){
				    	if(result.resExitoLocal == true){
							strParams = '?nomReporte=layouts';
							strParams += '&'+'nomParam1=rutaArchivo';
							strParams += '&'+'valParam1='+result.resRutaLocal;
							...
							Ext.Msg.alert('SET', 'No se genero el archivo.');
							NS.buscar();
						}
				    }
				});
    ...
    NS.storeEmpresas = new Ext.data.DirectStore({
		...
		directFn: EnvioTransferenciasAAction.obtenerEmpresas, 
	...
	NS.storeGrupo = new Ext.data.DirectStore({
		...
		root: '',
		directFn: EnvioTransferenciasAAction.llenarCombo, 
		idProperty: 'id', 
		fields: [
			 {name: 'id'},
			 {name: 'descripcion'},
	...
	//Store BancoEmisor
	  NS.storeBancoEmisor = new Ext.data.DirectStore({
		...
		root: '',
		paramOrder:['idUsuario'],
		directFn: EnvioTransferenciasAAction.obtenerBancoEmisor, 
		idProperty: 'id', 
		fields: [
			{name: 'id'},
			{name: 'descripcion'}
...
//						NS.storeEnvioTransGrid.baseParams.chkH2HSantander = true;
		...
		paramsAsHash: false,
		root: '',
		directFn: EnvioTransferenciasAAction.llenaComboCveControl, 
		idProperty: 'idStr', 
		fields: [
			{name: 'idStr'},
			{name: 'descripcion'}
		...
		root: '',
		paramOrder:['campoUno','campoDos','tabla','condicion','orden'],
		directFn: EnvioTransferenciasAAction.llenarComboGral, 
		idProperty: 'id', 
		fields: [
			{name: 'id'},
			{name: 'descripcion'}
	...
	//Store divisa
		...
		paramsAsHash: false,
		root: '',
		directFn: EnvioTransferenciasAAction.llenarComboDivisa, 
		idProperty: 'idDivisa', 
		fields: [
			{name: 'idDivisa'},
			{name: 'descDivisa'}
	...
	//Store para llenar el grid de envio de transferencias
  	NS.storeEnvioTransGrid = new Ext.data.DirectStore({
		...
		directFn: EnvioTransferenciasAAction.consultarPagos,
		fields: 
		[
			{name : 'noDocto'},
			{name : 'fecOperacion'},
	...
	NS.funActualizaCheq = function() {
		...
		EnvioTransferenciasAAction.actualizaMovtoCheqBenef(recSelect[0].get('noDocto'), parseInt(recSelect[0].get('noEmpresa')), 
	...
	NS.optEnvioBNMX = new Ext.form.RadioGroup({
		...
		items: [
	          ...
	          {boxLabel: 'TEF (Transferencia Electronica de Fondos)', name: 'optBNM', inputValue: 1},
	          {boxLabel: 'Citibank WorldLink', name: 'optBNM', inputValue: 2},  
	          {boxLabel: 'Mass Payment Citibank', name: 'optBNM', hidden: true, inputValue: 3},
	          {boxLabel: 'Flat File Citibank (Solo DLS)', name: 'optBNM', inputValue: 4},  
	          {boxLabel: 'Pay Link Citibank (Solo MN)', name: 'optBNM', inputValue: 5},
	          {boxLabel: 'Citibank ACH Credit', name: 'optBNM', inputValue: 6}
	     ]
	});
	...
	NS.storeNvoBanco = new Ext.data.DirectStore({
		...
		directFn: EnvioTransferenciasAAction.obtenerBancosBenef, 
		idProperty: 'id', 
		fields: [
			 {name: 'id'},
			 {name: 'descripcion'}
	...
	NS.storeNvaChequera = new Ext.data.DirectStore({
		...
		directFn: EnvioTransferenciasAAction.obtenerChequerasBenef, 
		idProperty: 'idStr', 
		fields: [
			 {name: 'idStr'},
			 {name: 'descripcion'}
