...
Ext.onReady(function(){
	var NS = Ext.namespace('apps.SET.Egresos.CompensarPropuestas');
    ...
    NS.storeGrupoEmpresas = new Ext.data.DirectStore({
		...
		root: '',
		directFn: CompensarPropuestasAction.llenarComboGrupoEmpresas, 
		idProperty: 'id', 
		fields: [
			 {name: 'id'},
			 {name: 'descripcion'},
	...
	/*   NS.storeDivision = new Ext.data.DirectStore({
		...
		paramOrder:['idUsuario'],
		root: '',
		directFn: CompensarPropuestasAction.llenarComboDivXEmp, 
		idProperty: 'id', 
		fields: [
			 {name: 'id'},
			 {name: 'descripcion'},
	...
	*/
    //Store para llenar el grid principal
  	 NS.storePagoPropuestas = new Ext.data.DirectStore({
		...
		directFn: CompensarPropuestasAction.consultarPropuestas,
		fields: [
			{name: 'empChe'},
			{name: 'cveControl'},
			{name: 'fecPropuesta'/*,
	...
	NS.storeDetaPro = new Ext.data.DirectStore({
		...
		directFn: CompensarPropuestasAction.consultarDetalle, 
		fields: [
	...
	NS.buscar = function() {
		...
		CompensarPropuestasAction.obtenerSaldosChequeras(NS.idGrupoEmpresa,NS.idDivision,NS.idUsuario, function(result, e){
                    	...
                    	click: {
                    		fn : function(e) {
                				...
                				var jsonString = Ext.util.JSON.encode(matrizValidar);	
									CompensarPropuestasAction.validarCpaVtaTransfer(jsonString, function(result, e){
										if(result!=null && result!=undefined && result!='')
										{
											for(var c=0;c<result.length;c=c+1)
											{
												...
												&& (result[c].mensaje2==null || result[c].mensaje2=='' || result[c].mensaje2==undefined))
												{
													...
													if(confirm(result[c].mensaje+', ¿Desea continuar?'))
													{
		                				...
		                				CompensarPropuestasAction.insetarZexpFact(jsonString4, apps.SET.FEC_HOY, NS.idUsuario, function(result, e){
		                					Ext.MessageBox.hide();
		                					if(result!==null && result!==undefined && result!==''){
...
NS.generarSubPropuesta = function(cveControlK, idGrupoFlujoK,idGrupoCupoK){
	...
	var idUsuario3 = 0;
	//alert(cveControlK);
	CompensarPropuestasAction.consultarDetallePropuestasNoPagadas(
			idGrupoFlujoK,idGrupoCupoK,cveControlK,idUsuario1,idUsuario2,idUsuario3, function(result, e){
				if(result!=null && result!=undefined && result!=''){
					...
					//Verificamos que no exista la nueva propuesta, si existe se va incrementando el consecutivo del final
					 CompensarPropuestasAction.validaCvePropuesta(nvaProp, function(result, e){
						 ...
						 if(result != null && result != '' && result != undefined) {								
							 ...
							 if(result.error != ''){
								...
								// alert('cve nueaaa '+nvaProp+'12 '+montoMaximo+' cve vieja'+cveControlK+' usuario '+NS.idUsuario+' fecha '+fecha);
								 ...
								 CompensarPropuestasAction.insertaSubPropuesta(parseFloat(montoMaximo.toFixed(2)), ''+nvaProp, ''+cveControlK, 
									 ...
									 if(result != null && result != '' && result != undefined) {								
										 ...
										 if(result.error != ''){
											...
											// alert(nvaProp+' nueve '+noDoctos+' ck'+cveControlK);
											 CompensarPropuestasAction.actualizaPropuesta(nvaProp, noDoctos, 
												...
												if(result != null && result != '' && result != undefined) {								
													 if(result.error != ''){
														 ...
														 CompensarPropuestasAction.actualizaMontoPropuesta(parseFloat(montoMaximo.toFixed(2)), cveControlK, 
