...
Ext.onReady(function (){
	...
	NS.lblMontos = new Ext.form.Label({
		text: 'Montos:',
		width: 110,
		height: 25,
		x: 15,
	...
	NS.txtMontoUno = new Ext.form.TextField({
		id: PF + 'txtMontoUno',
		name: PF + 'txtMontoUno',
		x: 130,
        y: 200,
	...
	NS.txtMontoDos = new Ext.form.TextField({
		id: PF + 'txtMontoDos',
		name: PF + 'txtMontoDos',
		x: 240,
        y: 200,
      					...
      					Ext.Msg.confirm('SET', '¿Desea realizar la solicitud de pago por el monto indicado? ' + NS.cambiarFormato(NS.importeAcumuladoFloat.toFixed(2), '$')  , function(btn){
      						if(btn == 'yes'){
      							NS.mascaraB.show();
									...
									NS.storeLlenarGridPagosTotal.baseParams.concepto=concepto;
									 NS.storeLlenarGridPagosTotal.baseParams.montoUno=montoUno;
									 NS.storeLlenarGridPagosTotal.baseParams.montoDos=montoDos;
									 NS.storeLlenarGridPagosTotal.baseParams.factura=factura;
	...
	NS.filterRowTres = new Ext.ux.grid.FilterRow({
		id: PF + 'filterRowTres',
		refilterOnStoreUpdate: true
	});
	...
	NS.storeLlenarGridPagosTotal = new Ext.data.DirectStore({ 
		...
		baseParams: {reglaNegocio: 0, fechaConIni: '', fechaConFin: '', fechaPropIni: '',
			...
			concepto:'' , montoUno:'' , montoDos:'' , factura:''},
		...
		listeners:{
			rowClick:{
				fn:function(grid){
        			...
        			NS.storeLlenarGridSolicitudPagos.baseParams.concepto=concepto;
        			NS.storeLlenarGridSolicitudPagos.baseParams.montoUno=montoUno;
        			NS.storeLlenarGridSolicitudPagos.baseParams.montoDos=montoDos;
        			NS.storeLlenarGridSolicitudPagos.baseParams.factura=factura;
	...
	//	idProperty: 'beneficiario',
		listeners:
		{
			load: function(s, records)
			{
				//console.log("pagos "+records.length);
				if(records.length == null || records.length <= 0)
					Ext.Msg.alert('SET', 'No existen pagos pendientes.');
			},
			exception:function(misc) {
			...
			dblclick:{
				fn:function(grid){
					...
					if(seleccion!= undefined){
						...
						NS.txtImporteTotal.setValue(NS.cambiarFormato((importeVencido + importePOrVencer).toFixed(2),'$'));
		                ...
		                if (NS.seleccionArray[NS.indiceActual] != null &&
							...
							for (var i = 0; i < vecAuxiliar.length; i++) {
								...
								NS.renglones[i].set('cveControl', NS.renglon['cveControl']);
							 	...
							 	NS.renglones[i].set('fechaFactura', NS.renglon['fechaFactura']);
...
//							NS.gridSolicitudPagosNivDos.getSelectionModel().selectRow(1);
	...
	NS.seleccionRenglonSolicitudPagosNivDos = new Ext.grid.CheckboxSelectionModel({
	  ...
	  NS.columnasSolicitudPagos = new Ext.grid.ColumnModel([
	    ...
	    {header: 'FACTURA', width: 150, dataIndex: 'cveControl', sortable: true,filter: {testFact: "/^{0}/i"}},
	    {header: 'INDICE', width: 70, dataIndex: 'index', sortable: true, hidden: true},
	    {header: 'FOLIO_SET', width: 120, dataIndex: 'factura', sortable: true, hidden: true},
	    {header: 'NO_DOC', width: 120, dataIndex: 'numeroDcoumento', sortable: true,filter: {testFac: "/^{0}/i"}},
	    {header: 'CLASE', width: 70, dataIndex: 'claseDocumento', sortable: true,},
	    {header: 'IMPORTE', width: 140, dataIndex: 'importeNeto', sortable: true, renderer: BFwrk.Util.rendererMoney ,align:'right'},
	    {header: 'CONCEPTO', width: 360, dataIndex: 'texto', sortable: true,filter: {testTex: "/^{0}/i"}},
	    {header: 'VENCIMIENTO', width: 100, dataIndex: 'fechaPropuesta', sortable: true},
		{header: 'DIVISA', width: 70, dataIndex: 'divisa', sortable: true, hidden: true},
	    {header: 'ESTATUS', width: 70, dataIndex: 'estatus', sortable: true,},
	    {header: 'FORMA DE PAGO', width: 120, dataIndex: 'formaPago' ,sortable: true},
	    {header: 'FECHA FACTURA', width: 120, dataIndex: 'fechaFactura' ,sortable: true},
...
//				NS.txtImporteTotal.setValue(NS.cambiarFormato((importeVencido + importePOrVencer),'$'));
	...
	NS.cargarPagosPorEmpresa= function(){
		if(NS.cmbReglaNegocioDos.getValue() == '' || NS.cmbReglaNegocioDos.getValue() == undefined || NS.cmbReglaNegocioDos.getValue() == null){
		   ...
		   concepto = NS.txtConcepto.getValue()+'';
		   montoUno = NS.txtMontoUno.getValue()+'';
		   montoDos = NS.txtMontoDos.getValue()+'';
		   factura= NS.txtFactura.getValue()+'';
		  ...
		  // console.log(reglaNegocio + " " + divisa + " " + indicadores + " " + numDoc + " " + tipoDoc);
			...
			NS.storeLlenarGridPagosTotal.baseParams.montoDos = montoDos;
			NS.storeLlenarGridPagosTotal.baseParams.factura = factura+'';
		//	Ext.Msg.alert("set",concepto+' '+montoUno+' '+montoDos+' '+factura);
	//		Ext.Msg.alert("set",NS.storeLlenarGridPagosTotal.baseParams.concepto);
	...
	NS.limpiarBuscar = function(bandera){
		...
		NS.txtMontoUno.setValue('');
		NS.txtMontoDos.setValue('');
			    	...
			    	handler:function(){
			    			...
			    			if(fechaPago=='' && idBancoBenef==0 && idChequeraBenef=='' && idBancoPag==0 && idChequeraPag=='' && refBancaria==''){
			    				...
			    				ConsultaPagosPendientesAction.ejecutarCambioDatosMovimiento(fechaPago,claves, idBancoBenef, idChequeraBenef, idBancoPag, idChequeraPag, refBancaria, ''+NS.tipoBusqueda, function(resultado, e) {
					    			...
					    			//NS.gridSolicitudPagos.store.removeAll();
					    			////NS.gridSolicitudPagos.getView().refresh();
			      					//NS.gridSolicitudPagosNivDos.load();
					    			Ext.Msg.alert('SET', resultado);
		      				   }); 
			    			}
   	    ...
   	    listeners:{
   	        	show:{
   	        			fn:function(){
   	        				...
   	        				if(NS.tipoBusqueda=='I'){
   	        				    NS.txtReferenciaBancaria.setDisabled(true);
   	        				}else{
   	        					NS.txtReferenciaBancaria.setDisabled(false);
   	        				}
   	        	    }
   	        	}
   	     }	
