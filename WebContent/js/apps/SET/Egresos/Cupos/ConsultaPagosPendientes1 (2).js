...
Ext.onReady(function (){
      						...
      						if(btn == 'yes'){
      							NS.mascaraB.show();
  						   		...
  						   		ConsultaPagosPendientesAction.crearPropuesta(jSonString, 
  						   			numeroEmpresa, 
  						   				NS.importeAcumuladoFloat, NS.cambiarFecha(''+NS.txtfechaPropuesta.getValue()), divisa, function(resultado, e){                          
  						   			NS.mascaraB.hide();
	...
	NS.storeCmbChqueras = new Ext.data.DirectStore({
		paramsAsHash: true,
		root: '',
		baseParams: {persona: '', idBanco: 0},
		paramOrder: ['persona', 'idBanco'],
	...
	NS.storeLlenarGridPagosTotal = new Ext.data.DirectStore({ 
		...
		baseParams: {reglaNegocio: 0, fechaConIni: '', fechaConFin: '', fechaPropIni: '',
			...
			equivalePersonaA:'' , equivalePersonaB:'' , grupoEmpresa:'' , jsonDocumentos: '' , jsonProveedores:'', provedoresIndividual:false, documentosIndividual:false},
		paramOrder: ['reglaNegocio', 'fechaConIni', 'fechaConFin', 'fechaPropIni', 'fechaPropFin',
				     ...
				     'equivalePersonaA','equivalePersonaB','grupoEmpresa','jsonDocumentos','jsonProveedores', 'provedoresIndividual', 'documentosIndividual'],
	...
	NS.storeLlenarGridSolicitudPagos = new Ext.data.DirectStore({
		...
		baseParams: {numeroEmpresa: 1, reglaNegocio: 0, fechaConIni: '', fechaConFin: '', fechaPropIni: '',
			...
			numDocA: '' ,numDocB: '' ,equivalePersonaA:'',equivalePersonaB:'', tipoDocBusq:'', grupoEmpresa: '' , jsonDocumentos: '' , jsonProveedores:'', provedoresIndividual:false, documentosIndividual:false}, //parametros de busqueda.
...
//							NS.gridSolicitudPagosNivDos.getSelectionModel().selectRow(1);
	...
	NS.storeLlenarGridSolicitudPagosNivDos = new Ext.data.DirectStore({
		...
		baseParams: {numeroEmpresa: 1, acredor: 1, reglaNegocio: 0, fechaConIni: '', fechaConFin: '', fechaPropIni: '',
			...
			equivalePersonaA:'', equivalePersonaB:'' ,grupoEmpresa:'' , jsonDocumentos: '' , jsonProveedores:'', documentosIndividual:false, beneficiario: ''
				},
		paramOrder: ['numeroEmpresa', 'acredor', 'reglaNegocio', 'fechaConIni', 'fechaConFin', 'fechaPropIni', 'fechaPropFin',
		             'divisa', 'indicadores', 'numDoc', 'tipoDoc' ,'numDocA','numDocB',
				     'equivalePersonaA','equivalePersonaB','grupoEmpresa','jsonDocumentos', 'jsonProveedores', 'documentosIndividual', 'beneficiario'],
			    	...
			    	handler:function(){
			    		if(NS.txtPagoPParcial.getValue() != null && NS.txtPagoPParcial.getValue() != '' && NS.txtPagoPParcial.getValue() != undefined){
			    			...
			    			ConsultaPagosPendientesAction.ejecutarPagoParcial(fechaPago,parseFloat(NS.unformatNumber(NS.txtPagoPImporte.getValue())) ,parseFloat(NS.unformatNumber(NS.txtPagoPParcial.getValue())) , ''+NS.txtPagoPFolioDet.getValue(), function(resultado, e) {
		      					...
		      					for(var i=0; i<NS.elementoSeleccionado.length;i++){
		      						if(NS.txtPagoPFolioDet.getValue()==NS.elementoSeleccionado[i]["factura"]){
		      							alert('Son iguales');
		      							delete NS.elementoSeleccionado[i];  
		      							
			      					}
		      					}
		      					var respaldo = NS.elementoSeleccionado;
		      					for(var i=0, j=0;i<respaldo.length;i++){
		      						if(respaldo[i]){
			      						NS.elementoSeleccionado[j]=respaldo[i];
			      						j++;
		      						}
		      					}
		      					 winPagoParcial.hide();
   	    ...
   	    listeners:{
   	        	show:{
   	        			fn:function(){
   	        				...
   	        				NS.txtPagoPImporte.setValue(NS.cambiarFormato(seleccion[0].get('importeNeto').toFixed(2)));
