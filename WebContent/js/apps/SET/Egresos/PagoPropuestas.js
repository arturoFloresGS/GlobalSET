...
Ext.onReady(function(){
	var NS = Ext.namespace('apps.SET.Egresos.PagoPropuestas');
    ...
    NS.storeGrupoEmpresas = new Ext.data.DirectStore({
		...
		root: '',
		directFn: PagoPropuestasAction.llenarComboGrupoEmpresas, 
		idProperty: 'id', 
		fields: [
			 {name: 'id'},
			 {name: 'descripcion'},
	...
	/*   NS.storeDivision = new Ext.data.DirectStore({
		...
		paramOrder:['idUsuario'],
		root: '',
		directFn: PagoPropuestasAction.llenarComboDivXEmp, 
		idProperty: 'id', 
		fields: [
			 {name: 'id'},
			 {name: 'descripcion'},
	...
	*/
    //Store para llenar el grid principal
  	 NS.storePagoPropuestas = new Ext.data.DirectStore({
		...
		directFn: PagoPropuestasAction.consultarPropuestas,
		fields: [
			{name: 'empChe'},
			{name: 'cveControl'},
			{name: 'fecPropuesta'/*,
	...
	NS.storeDetaPro = new Ext.data.DirectStore({
		...
		directFn: PagoPropuestasAction.consultarDetalle, 
		fields: [
	...
	NS.buscar = function() {
		...
		PagoPropuestasAction.obtenerSaldosChequeras(NS.idGrupoEmpresa,NS.idDivision,NS.idUsuario, function(result, e){
				    ...
				    items: [
				            {
			                	...
			                	x: 0,
			                	y: 0,
			                	style: 'background-color: #000; color: #000; font-weight: bold; padding:3px; border-radius: 3px;'
			                	
			                },
				            {
			                	xtype: 'label',
			                	text: 'PROPUESTA SIN OBSERVACIÓN',
			                	x: 50,
			                	y: 0,
			                ...
			                },
			                {
			                	xtype: 'label',
			                	text: '______',
			                	style: 'background-color: #090DFA; color: #090DFA; font-weight: bold; padding: 3px; border-radius: 3px;',
			                	x: 0,
			                	y: 25
				            ...
				            {
			                	xtype: 'label',
			                	text: 'PROPUESTA CON PAGOS REGRESADOS',
			                	x: 50,
			                	y: 25,
			                ...
			                },
			                {
			                	...
			                	x: 0,
			                	y: 50,
			                	style: 'background-color: #663351; color: #663351; font-weight: bold; padding: 3px; border-radius: 3px;'
			                	
			                },
				            {
			                	xtype: 'label',
			                	text: 'PROPUESTA CON PAGOS EN CHEQUE',
			                	x: 50,
			                	y: 50,
                    	...
                    	click: {
                    		fn : function(e) {
                				...
                				var jsonString = Ext.util.JSON.encode(matrizValidar);	
		                				...
		                				var chkAgrupa = Ext.getCmp(PF + 'chkAgrupar').getValue();
		                				//alert(chkAgrupa);
		                				
		                				PagoPropuestasAction.insetarZexpFact(jsonString, apps.SET.FEC_HOY, NS.idUsuario, chkAgrupa, function(result, e){
		                					Ext.MessageBox.hide();
		                						...
		                						if(result.estatus==true){
		                							if(result.excel !== ''){
		                								...
		                								if (result.ok != null || result.ok != '') {
		                									Ext.Msg.alert('Información SET', "Pago realizado correctamente." , function(btn){ 
		                										if (btn == 'ok'){
					                						    	NS.buscar();
					                						    }
		                									});
		                								} else {
		                									Ext.Msg.alert('Información SET', 'Hubo algun error' , function(btn){
					                						    if (btn == 'ok'){
					                						    	NS.buscar();
					                						    }
					                						});
...
NS.generarSubPropuesta = function(cveControlK, idGrupoFlujoK,idGrupoCupoK){
	...
	var idUsuario3 = 0;
	//alert(cveControlK);
	PagoPropuestasAction.consultarDetallePropuestasNoPagadas(
			idGrupoFlujoK,idGrupoCupoK,cveControlK,idUsuario1,idUsuario2,idUsuario3, function(result, e){
				if(result!=null && result!=undefined && result!=''){
					...
					//Verificamos que no exista la nueva propuesta, si existe se va incrementando el consecutivo del final
					 PagoPropuestasAction.validaCvePropuesta(nvaProp, function(result, e){
						 ...
						 if(result != null && result != '' && result != undefined) {								
							 ...
							 if(result.error != ''){
								...
								// alert('cve nueaaa '+nvaProp+'12 '+montoMaximo+' cve vieja'+cveControlK+' usuario '+NS.idUsuario+' fecha '+fecha);
								 ...
								 PagoPropuestasAction.insertaSubPropuesta(parseFloat(montoMaximo.toFixed(2)), ''+nvaProp, ''+cveControlK, 
									 ...
									 if(result != null && result != '' && result != undefined) {								
										 ...
										 if(result.error != ''){
											...
											// alert(nvaProp+' nueve '+noDoctos+' ck'+cveControlK);
											 PagoPropuestasAction.actualizaPropuesta(nvaProp, noDoctos, 
												...
												if(result != null && result != '' && result != undefined) {								
													 if(result.error != ''){
														 ...
														 PagoPropuestasAction.actualizaMontoPropuesta(parseFloat(montoMaximo.toFixed(2)), cveControlK, 
