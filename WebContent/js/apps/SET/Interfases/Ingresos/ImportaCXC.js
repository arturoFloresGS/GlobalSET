...
{
	var NS = Ext.namespace('apps.SET.Interfases.Ingresos.ImportaCXC'); //Nombre del archivo de trabajo
	...
	({		
		paramAsHash: false,
		root: '',
		paramOrder: ['txtIdEmpresa', 'estatus'],
		baseParams: {},
		directFn: CargaPagosAction.llenaGridCXC,
...
//		NS.storeEmpresas.removeAll();
	...
	NS.insertaRegistros = function() {
		...
		for(var i = 0; i < obtieneGrid.length; i++) {
			if(obtieneGrid[i].get('conceptoRechazo') == '') {
				var datos = {};
				datos.noEmpresa = obtieneGrid[i].get('noEmpresa');
				datos.noFactura = obtieneGrid[i].get('noFactura');
		...
		CargaPagosAction.insertaRegistrosCXC(jsonString, function(result, e) {
	...
	([	
	  	...
	  	{header: 'Factura', width: 100, dataIndex: 'noFactura', sortable: true},
	  	{header: 'Concepto', width: 150, dataIndex: 'concepto', sortable: true},
	  	{header: 'Fecha Factura', width: 80, dataIndex: 'fecFact', sortable: true},
	  	{header: 'Cliente', width: 150, dataIndex: 'razonSocial', sortable: true},
	  	{header: 'Importe', width: 100, dataIndex: 'importe', sortable: true, css: 'text-align:right;', renderer: BFwrk.Util.rendererMoney},
	  	{header: 'Divisa', width: 70, dataIndex: 'idDivisa', sortable: true},
	  	{header: 'Forma Pago', width: 80, dataIndex: 'idFormaPago', sortable: true},
	  	{header: 'Estatus', width: 70, dataIndex: 'estatusAlta', sortable:true},
	  	{header: 'Causa Rechazo', width: 200, dataIndex: 'conceptoRechazo', sortable: true}
	]);
				...
				fn: function(grid){
					...
					if(datosGrid.length > 0) {
						for(var i=0; i<datosGrid.length; i++) {
							if(datosGrid[i].get('idDivisa') == 'PESOS')
	...
	NS.agrupar = function() {
		...
		CargaPagosAction.armaPropuesta(jsonString, function(result, e) {
			...
			if (result !== null && result !== '') {
				...
				NS.storeLlenaGrid.baseParams.txtIdEmpresa = parseInt(noEmpresa);
	...
	NS.ImportaCXC = new Ext.FormPanel
	({
		title: 'Importación de CXC',
		width: 1300,
		height: 706,
		frame: true,
		padding: 10,
	...
	NS.ImportaCXC.setSize(Ext.get(NS.tabContId).getWidth(), Ext.get(NS.tabContId).getHeight());
});