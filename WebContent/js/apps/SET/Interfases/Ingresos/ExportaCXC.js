...
{
	var NS = Ext.namespace('apps.SET.Interfases.Ingresos.ExportaCXC'); //Nombre del archivo de trabajo
	...
	NS.exportaRegistros = function(){
		...
		ExportacionAction.exportaRegistrosCXC(NS.foliosExporta, function(resultado, e){
	...
	NS.storeLlenaGrid = new Ext.data.DirectStore({
		...
		directFn: ExportacionAction.llenaGridCXC,
		fields:
		[
		 	{name: 'noEmpresa'},
		 	{name: 'nomEmpresa'},
			...
			load: function(s, records) {
				...
				if (records.length == null || records.length <= 0)
					...
					for (var i = 0; i < records.length; i++){					
						...
						if (NS.estatus == 1 && records[i].get('causaRech') == '' && records[i].get('idTipoOperacion') != 1017) {
	...
	NS.ExportaCXC = new Ext.FormPanel
	({
		title: 'Exportación de CXC',
		width: 1300,
		height: 706,
		frame: true,
		padding: 10,
	...
	NS.ExportaCXC.setSize(Ext.get(NS.tabContId).getWidth(), Ext.get(NS.tabContId).getHeight());
});