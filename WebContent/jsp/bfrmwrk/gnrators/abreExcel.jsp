...
<%@page import="com.webset.set.cashflow.service.CashFlowActionImplements"%>
...
<%@page import="com.webset.set.financiamiento.action.AltaFinanciamientoAction"%>
<%@page import="com.webset.set.financiamiento.action.ReportePasivosFAction"%>
<%@page import="com.webset.set.financiamiento.action.ReporteAnalisisLineasCAction"%>
<%@page import="com.webset.set.financiamiento.action.AvalesGarantiasFCAction"%>
<%@page import="com.webset.set.derivados.action.ForwardsAction"%>
<%@ page session="true"%>
...
<%
	...
	try {
		...
		for (; request.getParameter(sNomVarParm) != null
				&& !request.getParameter(sNomVarParm).toString().equals(""); i++, sNomVarParm = "nomParam"
						+ i, sNomValParm = "valParam" + i)
		...
		if (sNombreRep.equals("prueba")) {
			...
			wb = consulta.reportePersonas2(params.get("tipoPersona").toString(),
					params.get("tipoPersona2").toString(), params.get("tipoPersona3").toString(),
					params.get("tipoPersona4").toString(), params.get("tipoPersona5").toString(), context);

			if (wb == null) {
				System.out.println("es nulo el resultado!!!!");
			}
		} else if (sNombreRep.equals("excelPagosP")) {
			AlertaVencimientoAction consultaEx = new AlertaVencimientoAction();
			Gson jso = new Gson();
			wb = consultaEx.reportePagosP(params.get("tipo").toString());
			if (wb == null) {
				System.out.println("es nulo el resultado!!!!");
			}
			...
			wb = mantenimientoChequerasAction.reporteChequeras(params.get("tipoChequera"),
					params.get("empresa"), context);
		...
		} else if(sNombreRep.equals("excelflujo2")){
			CashFlowActionImplements cashFlowAction = new CashFlowActionImplements(); 
			 
			wb=cashFlowAction.reporteExcels2(params.get("grupo").toString(),params.get("Noempresa").toString(),params.get("empresa").toString()
		 			,params.get("fechaIni").toString(),params.get("fechaFin").toString(),params.get("reporte").toString(),params.get("Nogrupo").toString(),context); 
		}else if (sNombreRep.equals("interfaces")) {
			CargaPagosAction cargaPagosAction = new CargaPagosAction();
			wb = cargaPagosAction.reporteInterfaces(params.get("tipoValor"), params.get("fecHoy"),
					params.get("usuarioAlta"), params.get("idEmpresa"), params.get("estatus"),
					params.get("fecIni"), params.get("fecFin"), context);
			...
			wb = consultasAction.reporteFondeo(params.get("fInicio").toString(), params.get("fFin").toString(),
					params.get("bandera").toString(), params.get("usuario").toString(), context);
		} else if (sNombreRep.equals("excelImportaciones")) {
			ConsultasAction consultasAction = new ConsultasAction();
			wb = consultasAction.reporteCXP(params.get("noEmpresa").toString(), params.get("fecIni").toString(),
					params.get("fecFin").toString(), params.get("estatus").toString(),
					params.get("tipoReporte").toString(), context);
			...
			wb = pagoPropuestasAction.consultaPagosExcel(params.get("clave"), context);
			...
			AplicacionDescuentosPropuestasAction descuentosAction = new AplicacionDescuentosPropuestasAction();
			wb = descuentosAction.consultaPropuestasDescuentos(params.get("claveP"), params.get("claveD"),
					context);
			...
			wb = barridosFondeosRepAction.obtenerExcelBarridosFondeos(Integer.parseInt(params.get("noEmpresa")),
					Integer.parseInt(params.get("idUsuario")), params.get("fecha"), context);
			if (wb == null) {
				System.out.println("es nulo el resultado!!!!");
			}
		} else if (sNombreRep.equals("excelCuadreFondeo")) {
			BarridosFondeosRepAction barridosFondeosRepAction = new BarridosFondeosRepAction();
			wb = barridosFondeosRepAction.obtenerExcelCuadreFeondeo(Integer.parseInt(params.get("noEmpresa")),
					Integer.parseInt(params.get("idUsuario")), params.get("fecha"), context);
			if (wb == null) {
				System.out.println("es nulo el resultado!!!!");
			}
			...
			wb = cartasPorEntregarAction.reporteCartasEmitidas(params.get("folio"), params.get("idBanco"),
					params.get("tipo"), params.get("estatus"), params.get("fechaIni"), params.get("fechaFin"),
					context);
		} else if (sNombreRep.equals("MovimientoPosicionBancaria")
				|| sNombreRep.equals("MovimientoPosicionBancaria2") || sNombreRep.equals("excelMovtosTesoreria")
				|| sNombreRep.equals("excelDiarioTesoreria") || sNombreRep.equals("excelFecValorXEmp")
				|| sNombreRep.equals("excelFecValorXCta") || sNombreRep.equals("excelReporteSaldos")) {
			...
			wb = inversionesRepAction.obtenerExcelInvEstablecidas(Integer.parseInt(params.get("noEmpresa")),
					params.get("idDivisa"), params.get("fecha"), params.get("fechaFin"),
			...
			wb = consultasAction.obtenerDatosExcelReporteSaldosS(params.get("idEmpresa"),
					params.get("nomEmpresa"), params.get("idBancoInf"), params.get("idBancoSup"),
					params.get("idChequera"), params.get("tipoChequera"), params.get("estatusTipoEmpresa"),
					params.get("usuario"), params.get("empresas"), context);
			if (wb == null) {
				System.out.println("es nulo el resultado!!!!");
			}
		} else if (sNombreRep.equals("excelAmortizaciones")) {
			AltaFinanciamientoAction altaFinanciamientoAction = new AltaFinanciamientoAction();
			wb = altaFinanciamientoAction.reporteAmortizaciones(params.get("contrato"),
					params.get("disposicion"), params.get("montoAutorizado"), params.get("banco"),
					params.get("montoDisposicion"), params.get("divisa"), params.get("fechaInicio"),
					params.get("fechaFin"), params.get("tasa"), context);

		}
		 else if (sNombreRep.equals("excelPasivosFinancieros")) {
				ReportePasivosFAction reportePasivosFAction = new ReportePasivosFAction();
				wb = reportePasivosFAction.reportePasivosFinancieros(request.getParameter("hdJasonP"),context);
			}
		 else if (sNombreRep.equals("excelAnalisisLineas")) {
				ReporteAnalisisLineasCAction reporteAnalisisLineasCAction = new ReporteAnalisisLineasCAction();
				wb = reporteAnalisisLineasCAction.excelAnalisisLineasCredito(request.getParameter("hdJasonExcel"),context);
			}
		 else if (sNombreRep.equals("excelResumenAnalisisLineas")) {
			 ReporteAnalisisLineasCAction reporteAnalisisLineasCAction = new ReporteAnalisisLineasCAction();
				wb = reporteAnalisisLineasCAction.excelAnalisisLineasCreditoResumen(request.getParameter("hdJasonRExcel"),context);
			}
		 else if (sNombreRep.equals("excelAvaladas")) {
			AvalesGarantiasFCAction avalesGarantiasFCAction = new AvalesGarantiasFCAction();
				wb = avalesGarantiasFCAction.excelAvaladas(request.getParameter("hdJasonAva"),context);
			}
		 else if (sNombreRep.equals("excelAvalistas")) {
			 AvalesGarantiasFCAction avalesGarantiasFCAction = new AvalesGarantiasFCAction();
				wb = avalesGarantiasFCAction.excelAvalistas(request.getParameter("hdJasonAv"),context);
			}
