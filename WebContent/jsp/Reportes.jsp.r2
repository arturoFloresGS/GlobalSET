<%@page import="com.webset.set.financiamiento.action.GastosFinanciamientoCAction"%>
<%@page import="com.webset.set.financiamiento.dao.GastosFinanciamientoCDao"%>
...
try{
	...
	lista = accion.obtenerReporteArbolEstruct(Integer.parseInt(noEmpresa), Integer.parseInt(idUsuario), context);
	...
	System.out.println("Entra al jsp 1" + lista);
		...
		String idDisposicion=request.getParameter("idDisposicion");
		lista = gastosFinanciamientoC.obtenerReporteGastos(idLinea, Integer.parseInt(idDisposicion), context);
		}
	//
	else if (sNombreRep.equals("ReporteBarridosFondeosFec"))
	{
		lista = accion.obtenerReporteBarridosFeondeos(Integer.parseInt(noEmpresa), Integer.parseInt(idUsuario), sFecha, context);
		System.out.println("Report" + lista);
	}	         
	else if (sNombreRep.equals("ReporteCuadreFondeo"))
		lista2 = accion.obtenerReporteCuadreFeondeo(Integer.parseInt(noEmpresa), Integer.parseInt(idUsuario), sFecha, context);
	else if (sNombreRep.equals("ReporteFondeoBarridoAuto")){
		lista = accion.generarReporteAutomatico(hdJason, Integer.parseInt(noEmpresa), 
				Integer.parseInt(idArbol), Integer.parseInt(idBanco), idDivisa, idChequera, false, tipoOperacion, false, "0", Integer.parseInt(idUsuario), 
				sNomEmpresa, context);
		lista = accion.obtenerReporteFondeo(Integer.parseInt(idUsuario),"", context);
	...
	if (sNombreRep.equals("ReporteFondeo") || sNombreRep.equals("ReporteFondeoBarrido") || sNombreRep.equals("ReporteBarridosFondeosFec")
			|| sNombreRep.equals("ReporteCuadreFondeo") || sNombreRep.equals("ReporteArbolEstructura")
